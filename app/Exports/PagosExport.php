...
public function GetPagos($sucursal_id){
        ...
        $datos_pagos_query = pagos_facturas::select(
            ...
            DB::raw('CASE
                        WHEN pagos_facturas.id_venta_insumos IS NOT NULL AND pagos_facturas.id_venta_insumos <> 0 THEN "venta insumos"
            ...
            DB::raw('CASE
                        ...
                        WHEN pagos_facturas.id_compra IS NOT NULL AND pagos_facturas.id_compra <> 0 THEN (-1*(pagos_facturas.monto_compra + pagos_facturas.actualizacion))
                        WHEN pagos_facturas.id_gasto IS NOT NULL AND pagos_facturas.id_gasto <> 0 THEN (-1 * pagos_facturas.monto_gasto)
            ...
            DB::raw('CASE
                        ...
                        WHEN pagos_facturas.id_venta_insumos IS NOT NULL AND pagos_facturas.id_venta_insumos <> 0 THEN (pagos_facturas.deducciones)
                        ELSE 0
                    END AS deduccion'),
            DB::raw('CASE
                        WHEN pagos_facturas.id_venta_insumos IS NOT NULL AND pagos_facturas.id_venta_insumos <> 0 THEN (pagos_facturas.monto + pagos_facturas.recargo + pagos_facturas.iva_pago + pagos_facturas.iva_recargo - pagos_facturas.deducciones)
