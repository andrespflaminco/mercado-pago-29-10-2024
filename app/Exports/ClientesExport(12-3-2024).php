...
class ClientesExport implements FromCollection, WithHeadings, WithCustomStartCell, WithTitle, WithStyles
{
    ...
    public function collection()
    {
       ...
       // dd($this->sucursal_id);
        ...
        $data = ClientesMostrador::leftjoin('lista_precios','lista_precios.id','clientes_mostradors.lista_precio')->join('users','users.id','clientes_mostradors.creador_id')
        ...
        $data = $data->whereIn('clientes_mostradors.creador_id', $sucursalIds);    
            ...
            ,'clientes_mostradors.barrio','clientes_mostradors.localidad','clientes_mostradors.provincia','clientes_mostradors.codigo_postal','lista_precios.nombre as lista',ClientesMostrador::raw('DATE_FORMAT(clientes_mostradors.last_sale,"%d/%m/%Y") as fecha'), 'clientes_mostradors.observaciones')
    ...
    {
      return ["COD CLIENTE","NOMBRE","SUCURSAL","CUIT","TELEFONO","EMAIL","CALLE","ALTURA","PISO","DEPTO","BARRIO","LOCALIDAD","PROVINCIA","CODIGO POSTAL","LISTA PRECIOS","ULTIMA COMPRA","OBSERVACIONES"];
    }
