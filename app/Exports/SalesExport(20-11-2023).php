...
class SalesExport implements FromCollection, WithHeadings, WithCustomStartCell, WithTitle, WithStyles
{
    ...
    protected $usuarioSeleccionado, $clienteId,$estado_pago,$estado, $dateFrom, $dateTo;

    function __construct($usuarioSeleccionado, $clienteId, $estado_pago,$estado, $metodo_pago, $f1, $f2) {
    ...
    public function collection()
    {
      ...
      ->select('sales.id', Sale::raw("DATE_FORMAT(sales.created_at,'%d-%m-%Y %H:%i') as fecha"),'cm.nombre as nombre_cliente','sales.status','sales.items',Sale::raw("(sales.subtotal) as subtotal"),Sale::raw("IFNULL(sales.descuento,0) as descuento"),Sale::raw("(IFNULL(sales.recargo,0) ) as recargo"),Sale::raw("(IFNULL(sales.iva,0) ) as iva"),Sale::raw("(sales.subtotal - IFNULL(sales.descuento,0) + IFNULL(sales.recargo,0) + IFNULL(sales.iva,0) ) as total"),'m.nombre as metodo_pago','u.name as user','sales.deuda')
    ...
    {
      return ["FOLIO","FECHA","CLIENTE","ESTADO","CANT. ITEMS","SUBTOTAL","DESCUENTO","RECARGO","IVA","TOTAL","FORMA DE PAGO","USUARIO","DEUDA"];
    }
