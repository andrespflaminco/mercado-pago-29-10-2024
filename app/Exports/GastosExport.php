...
class GastosExport implements FromCollection, WithHeadings, WithCustomStartCell, WithTitle, WithStyles
{
    ...
    protected $search,$categoria_filtro,$etiquetas_filtro,$forma_pago_filtro,$metodo_pago_filtro, $dateFrom, $dateTo;


    function __construct($search,$categoria_filtro,$etiquetas_filtro,$metodo_pago_filtro,$forma_pago_filtro, $f1, $f2) {
        
       // dd($search,$categoria_filtro,$etiquetas_filtro,$forma_pago_filtro, $f1, $f2);
        
        $this->search = $search;
        $this->categoria_filtro = $categoria_filtro;
        $this->etiquetas_filtro = explode(",", $etiquetas_filtro);
    ...
    ->select('gastos.nombre','gastos_categorias.nombre as nombre_categoria','gastos.monto',gastos::raw('DATE_FORMAT(gastos.created_at, "%d/%m/%Y") AS fecha_formateada'),'gastos.etiquetas as nombre_etiqueta','bancos.Nombre as nombre_banco','gastos.forma_pago')
    ...
    if (array_key_exists(0, $this->etiquetas_filtro) && $this->etiquetas_filtro[0] !== "0") {
    ...
    if($this->forma_pago_filtro != "0") {
      ...
      $gastos = $gastos->where('gastos.forma_pago','LIKE',$this->forma_pago_filtro);
    ...
    {
      return ["NOMBRE","CATEGORIA","MONTO","FECHA","ETIQUETA","METODO DE PAGO","FORMA DE PAGO"];
    }
