							...
							//ShouldAutoSize, WithEvents //WithColumnFormatting
{
   ...
   // método para obtener de base de datos la info a exportar
    public function collection()
    {
    	$data = [];
    		...
    		$to = Carbon::parse($this->dateTo)->format('Y-m-d').' 23:59:59';
				...
				->select('sale_details.sale_id',SaleDetail::raw("DATE_FORMAT(sale_details.created_at,'%d-%m-%Y %H:%i') as fecha"),'sale_details.product_barcode as barcode','sale_details.product_name as product','categories.name as nombre_categoria','sale_details.price','sale_details.quantity',SaleDetail::raw('( ((sale_details.price*sale_details.quantity) + IFNULL(sale_details.recargo, 0) - IFNULL(sale_details.descuento, 0))*(sale_details.iva)) as iva'),SaleDetail::raw('(IFNULL(sale_details.recargo, 0)) as recargo'),SaleDetail::raw('(IFNULL(sale_details.descuento, 0)) as descuento'),SaleDetail::raw('((sale_details.price*sale_details.quantity) + ( ((sale_details.price*sale_details.quantity) + IFNULL(sale_details.recargo, 0) - IFNULL(sale_details.descuento, 0))*(IFNULL(sale_details.iva, 0))) + IFNULL(sale_details.recargo, 0) - IFNULL(sale_details.descuento, 0)) as total'),'cl.nombre as nombre_cliente','users.name as nombre_usuario','m.nombre as nombre_metodo_pago','sa.status','s.nombre as almacen')
    ...
    {
    	return ["ID VENTA","FECHA","CODIGO PROD","PRODUCTO","CATEGORIA","PRECIO", "CANTIDAD","IVA","RECARGO FINANC.","DESCUENTO","TOTAL","CLIENTE" ,"VENDEDOR","METODO DE PAGO","ESTADO VENTA","ALMACEN"];
    }
