...
class ProductsEjemploExport implements FromCollection, WithHeadings, WithCustomStartCell, WithTitle, WithStyles
{
    
    private $comercio_id; // declaras la propiedad

    public function __construct($comercio_id) 
    {
...
///////////////////////////   PRECIOS    ///////////////////////////////////////////////
    ...
    /////////////////////////// STOCKS //////////////////////////////////////////////
      ...
      (SELECT  stock_real FROM productos_stock_sucursales where product_id = P.id and sucursal_id = 0 AND eliminado = 0 LIMIT 1) ) AS BASE");
      ...
      $q_s2 =  DB::raw($q_ss);
        ...
        $select =  ['P.name',DB::raw( 'CASE WHEN P.producto_tipo = "s" THEN "simple" ELSE "variable"
                END'),'PV.variaciones','P.barcode','PV.codigo_variacion',$q_c1, $q_p1, $q_p1_interno, $q_p2, $q_s1, $q_s2, 'P.alerts', 'P.stock_descubierto','c.name as category','pr.nombre as nombre_proveedor','a.nombre as almacen',DB::raw( 'CASE WHEN P.tipo_producto =1 THEN "compra" ELSE "produccion"
                ...
                END'),'i.name as imagen','P.etiquetas'];
        }

        if(  !empty($q_ss) && empty($q_lp) ) {

        $select =  ['P.name',DB::raw( 'CASE WHEN P.producto_tipo = "s" THEN "simple" ELSE "variable"
                END'),'PV.variaciones','P.barcode','PV.codigo_variacion',$q_c1, $q_p1,$q_p1_interno, $q_s1, $q_s2, 'P.alerts', 'P.stock_descubierto','c.name as category','pr.nombre as nombre_proveedor','a.nombre as almacen', DB::raw( 'CASE WHEN P.tipo_producto =1 THEN "compra" ELSE "produccion"
                END'), DB::raw( 'CASE WHEN P.mostrador_canal =1 THEN "si" ELSE "no"
                END'),DB::raw( 'CASE WHEN P.ecommerce_canal =1 THEN "si" ELSE "no"
                END'),DB::raw( 'CASE WHEN P.wc_canal =1 THEN "si" ELSE "no"
                END'),'i.name as imagen','P.etiquetas'];
        ...
        if( empty($q_ss) && !empty($q_lp) ) {
        $select =  ['P.name',DB::raw( 'CASE WHEN P.producto_tipo = "s" THEN "simple" ELSE "variable"
                END'),'PV.variaciones','P.barcode','PV.codigo_variacion', $q_c1, $q_p1,$q_p1_interno, $q_p2, $q_s1, 'P.alerts', 'P.stock_descubierto','c.name as category','pr.nombre as nombre_proveedor','a.nombre as almacen',DB::raw( 'CASE WHEN P.tipo_producto =1 THEN "compra" ELSE "produccion"
                END'), DB::raw( 'CASE WHEN P.mostrador_canal =1 THEN "si" ELSE "no"
                END'),DB::raw( 'CASE WHEN P.ecommerce_canal =1 THEN "si" ELSE "no"
                END'),DB::raw( 'CASE WHEN P.wc_canal =1 THEN "si" ELSE "no"
                END'),'i.name as imagen','P.etiquetas'];
        ...
        if( empty($q_ss) && empty($q_lp) ) {
        $select =  ['P.name',DB::raw( 'CASE WHEN P.producto_tipo = "s" THEN "simple" ELSE "variable"
                END'),'PV.variaciones','P.barcode','PV.codigo_variacion',$q_c1, $q_p1,$q_p1_interno, $q_s1, 'P.alerts', 'P.stock_descubierto','c.name as category','pr.nombre as nombre_proveedor','a.nombre as almacen',DB::raw( 'CASE WHEN P.tipo_producto =1 THEN "compra" ELSE "produccion"
                END'), DB::raw( 'CASE WHEN P.mostrador_canal =1 THEN "si" ELSE "no"
                END'),DB::raw( 'CASE WHEN P.ecommerce_canal =1 THEN "si" ELSE "no"
                END'),DB::raw( 'CASE WHEN P.wc_canal =1 THEN "si" ELSE "no"
                END'),'i.name as imagen','P.etiquetas'];
    ...
    public function headings() : array
    {
     ...
     $this->casa_central_id = $this->comercio_id;
      ...
      array_push($header, "INV MINIMO","MANEJA STOCK","CATEGORIA","MARCA","PROVEEDOR","ORIGEN","UNIDAD DE MEDIDA","VENTA_MOSTRADOR","VENTA_ECOMMERCE","VENTA_WOCOMMERCE","ETIQUETAS","CANTIDAD POR UNIDAD","CODIGO PROVEEDOR");
