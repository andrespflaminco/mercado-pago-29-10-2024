    ...
    try {
      for ($i = 0; $i < count($filasTotales) ; $i++) {
       ...
       if($this->accion != 1){
        ...
        if($this->accion == 2){
        $this->SetPreciosInternosYCostos($filasTotales[$i]['precio_interno'],$filasTotales[$i]['costo'],$product,$this->referencia_id,$comercio_id);
        }
       ...
       if($this->accion == 1){
        ...
        $precio_interno_simple  = $this->PrecioInternoSimple($filasTotales[$i]['precio_interno'],$producto_tipo);
        $precio_interno_variable = $this->PrecioInternoVariable($filasTotales[$i]['precio_interno'],$producto_tipo);
...
public function proveedores($nombre_proveedor, $comercio_id)
  ...
  public function PrecioInternoSimple($row,$producto_tipo) {
      if($producto_tipo == "s") {
          return $row ?? 0;
      } else {
          return 0;
  ...
  public function PrecioInternoVariable($row,$producto_tipo) {
      if($producto_tipo == "v") {
          return $row ?? 0;
      } else {
          return 0;
...
function obtenerIdEtiqueta($nombre_etiqueta,$comercio_id)
{
    ...
    public function StoreUpdateCostosYPrecioInterno($product,$costo,$precio_interno,$producto_tipo,$pvd_datos) {
    ...
    public function SetPreciosInternosYCostos($precios_internos,$costo,$product,$referencia_id,$comercio_id){
            
        // producto simple
        if($product->producto_tipo == "s") {
        $precio_interno  = $this->PrecioInternoSimple($precios_internos,$product->producto_tipo);
        $this->StoreUpdateCostosYPrecioInterno($product,$costo ?? 0,$precio_interno,$product->producto_tipo,null);
        }

        // producto variable
        if($product->producto_tipo == "v") {
        $precio_interno = $this->PrecioInternoVariable($precios_internos,$product->producto_tipo);
        ...
        $this->StoreUpdateCostosYPrecioInterno($product,$costo ?? 0,$precio_interno,$product->producto_tipo,$pvd_datos);
        }
