...
class ImportJob implements ShouldQueue
{
 ...
 //   $filasTotales = $this->data;
    ...
    $comercio_id = $imp->comercio_id;
     ...
     // Verificamos si está bloqueado para evitar múltiples ejecuciones simultáneas
    if ($imp->locked) {
    \Log::error('Proceso de importación ya está en ejecución: ' . $importId);
        return; // Salir si ya está corriendo
    }   
    \Log::error('Importacion: ' . $imp->id);
    // Bloqueamos el proceso para que no se ejecute de nuevo
    $imp->update(['locked' => true]);
    ...
    if($imp->terminado == 0){
      ...
      for ($i = 0; $i < count($filasTotales) ; $i++) {
       ...
       if($this->accion == 1){
        ...
        $array       = $this->subcategorias($filasTotales[$i], $comercio_id,$exists,$columna,$array); //////////// SUBCATEGORIA //////////////
    ...
    //    $array       = $this->pesable($filasTotales[$i],$comercio_id,$exists,$columna,$array); //////////// PESABLE //////////////
        ...
        if ($existe_producto_validacion)
        {
          ...
          if($product != null) {          
            ...
            $this->StoreUpdateDescuentos($filasTotales,$i,$product,$comercio_id,$columna,$exists,0);
        ...
        if($array['producto_tipo'] == "v"){
            $this->StoreUpdateDescuentos($filasTotales,$i,$product,$comercio_id,$columna,$exists,$this->referencia_id);
        }
        
        $this->FuncionesVariaciones($filasTotales[$i],$i,$product,$comercio_id,$columna,$exists,$array,$configuracion_precio_interno);

        $this->StoreUpdateStock($filasTotales[$i],$i,$product,$comercio_id,$id_almacen,$columna,$exists,"casa central",0);

        $this->StoreUpdatePrecios($filasTotales[$i],$i,$product,$comercio_id,$columna,$exists,"base",0);
        
        $this->StoreIVAProducto($filasTotales[$i],$i,$product,$comercio_id,$id_almacen,$columna,$exists,"iva",0);

        $SS = sucursales::join('users','users.id','sucursales.sucursal_id')
        ->where('casa_central_id', $comercio_id)->where('eliminado',0)->get();

        $LP = lista_precios::where('comercio_id', $comercio_id)->get();
       ...
       foreach($LP as $lp) {
        ...
        $lista_id = $lp->id;
        $nombre_lista = str_replace(' ', '_', $lista);
        
        $this->StoreUpdatePrecios($filasTotales[$i],$i,$product,$comercio_id,$columna,$exists,$nombre_lista,$lista_id);
        }

      foreach($SS as $ss) {
        
        $sucursal = strtolower($ss->name);
        $nombre_sucursal = str_replace(' ', '_', $sucursal);
        
        // STOCK
        $this->StoreUpdateStock($filasTotales[$i],$i,$product,$comercio_id,$id_almacen,$columna,$exists,$nombre_sucursal,$ss->id);

        // IVA
        $this->StoreIVAProducto($filasTotales[$i],$i,$product,$comercio_id,$id_almacen,$columna,$exists,$nombre_sucursal,$ss->id);
        }
        
        $this->ActualizarReglaDePreciosDB($product->id,$this->referencia_id,$product);
...
public function subcategorias($row,$comercio_id,$exists,$columna,$array)
  {
    $nombre_columna = $columna['subcategoria'];
    
    if($nombre_columna != "0"){ 

    $nombre_subcategoria = $row[$nombre_columna];
    $categoria_id = 1; 
    
    $sub = Subcategoria::where('nombre',$nombre_subcategoria)->where('comercio_id',$comercio_id)->where('eliminado',0)->first();    // si existe usa ese
    
    if(isset($array['category_id'])){
    $categoria_id = $array['category_id'];
    } else {
    if($exists != null) { $categoria_id = $exists->category_id;  } else {$categoria_id = 1;}  
    }
    ...
    $subcategoria_id = $this->CrearSubcategoria($nombre_subcategoria,$categoria_id,$comercio_id);
    }
    } else {
    $subcategoria_id = 1;    
    }

    $array = $this->agregarElementoAlArray($array, 'subcategoria_id', $subcategoria_id);

    } else {
    if($exists == null){ $array = $this->agregarElementoAlArray($array, 'subcategoria_id', 1);}
    }
...
public function CrearSubcategoria($nombre,$categoria_id,$comercio_id){
    // si no existe lo crea
    $c = Subcategoria::create([
        'comercio_id' => $comercio_id,
        'nombre' => $nombre,
...
public function almacen($row,$comercio_id,$exists,$columna)
  ...
  if(isset($array['cost'])){
      $costo_original = $array['cost'];
      $costo_pasar = $this->calcularCostoDespuesDeDescuento($product->id,$referencia_variacion, $costo_original);
  } else {
      $costo_pasar =  $this->GetCostoParaReglaPrecios($columna,$row,$product,$referencia_variacion);   
  }
  ...
  if($regla == 2){
  $costo = $this->GetCostoParaReglaPrecios($columna,$row,$product,$referencia_variacion);    
...
function obtenerIdEtiqueta($nombre_etiqueta,$comercio_id)
{
    ...
    public function GetCostoParaReglaPrecios($columna,$filasTotales,$product,$referencia_variacion){
    ...
    if($product->producto_tipo == "s"){
        $cost = $product->cost;
        $costo_despues_descuento = $this->calcularCostoDespuesDeDescuento($product->id,$referencia_variacion,$cost);
        return $costo_despues_descuento;
    } else {
    ...
    var_dump($product->id,$referencia_variacion,$cost);
    $costo_despues_descuento = $this->calcularCostoDespuesDeDescuento($product->id,$referencia_variacion,$cost);
    var_dump("Costo despues descuento " . $costo_despues_descuento);
    return $costo_despues_descuento;
    }  
    ...
    public function GetMargenParaReglaPrecios($product,$filasTotales,$lista_id,$nombre_columna_margen_regla_precio){
    ...
    $porcentaje_defecto_lista = $this->GetPorcentajeDefectoDB($lista_id,$product->comercio_id);
    return $productos_lista_precios ? $productos_lista_precios->porcentaje_regla_precio: $porcentaje_defecto_lista;    
    } else {
    ...
    public function StoreUpdatePrecioPorcentaje($filasTotales, $product, $comercio_id, $columna, $exists, $costo, $porcentaje_regla_precios, $lista_id, $nombre_lista)
    {
        // Obtener el nombre de la columna de margen de regla de precio
        ...
        $nombre_columna_costo = $columna['costo'];
         ...
         $porcentaje_regla_precios = $this->GetMargenParaReglaPrecios($product,$filasTotales,$lista_id,$nombre_columna_margen_regla_precio);        
        
        // Llamar a la función que maneja el almacenamiento o actualización
        $this->StoreUpdatePrecioPorcentajeDB($exists, $nombre_columna_costo, $nombre_columna_margen_regla_precio, $lista_id, $product, $comercio_id, $costo, $porcentaje_regla_precios, $porcentaje_regla_precios);
    }
        ...
        // Si no existe el producto y no tiene alguna de las dos columnas
          ...
          $precio_viejo = $this->getPrecioAnterior($product, $comercio_id, $lista_id);
          
          // Si la columna es 0 y no existe, crearlo con valor 0
          if ($nombre_columna_costo == "0" && $nombre_columna_margen_regla_precio == "0") {
             ...
             $this->crearPrecioInicial($product, $comercio_id, $lista_id,$porcentaje_regla_precio);
          } else {
    ...
    public function StoreUpdateDescuentosDB($filasTotales,$i,$product,$comercio_id,$columna,$exists,$nro_descuento,$descuento_id,$referencia_id){
        
        $nombre_columna =   $descuento_id  ."_descuento_costo_" . $nro_descuento;
        
        $nombre_columna_descuento = $columna[$nombre_columna];
        
        //var_dump($nombre_columna_descuento);
        if($nombre_columna_descuento != "0"){
        $descuento = $filasTotales[$nombre_columna_descuento];
        
        $group = productos_descuentos::updateOrCreate(
                    [
                      'product_id'          => $product->id,
                      'referencia_variacion' => $referencia_id,
                      'nro_descuento'               => $nro_descuento,
                      'lista_descuento_id' => $descuento_id
                    ],
                    [
                      'descuento'           => $descuento
                    ]
    ...
    public function StoreUpdatePrecios($filasTotales,$i,$product,$comercio_id,$columna,$exists,$nombre_lista,$lista_id){
        
        if($lista_id == 0){
        $nombre_columna = 'precio'; // Columna del precio
        $nombre_lista = 'base';            
        } else {
        $nombre_columna =   $lista_id  ."_precio_" . $nombre_lista;
        }

        $nombre_columna_precio = $columna[$nombre_columna];
  
        $regla = $this->GetReglaPrecioDB($lista_id,$comercio_id);
        $costo = $this->GetCostoParaReglaPrecios($columna,$filasTotales,$product,$this->referencia_id);
        
        if($regla == 1){
          $this->StoreUpdatePrecioFijo($filasTotales,$product,$comercio_id,$columna,$exists,$nombre_columna,$lista_id,$costo);  
        }

        if($regla == 2){
   
          $porcentaje_regla_precios = 0;
          //var_dump('costo '.$costo , 'porcentaje_regla_precios '.$porcentaje_regla_precios);
          $this->StoreUpdatePrecioPorcentaje($filasTotales,$product,$comercio_id,$columna,$exists,$costo,$porcentaje_regla_precios,$lista_id,$nombre_lista);  
        }
        
    
    }

    public function SetSucursalOrCero($sucursal_id,$comercio_id){
        if($sucursal_id == $comercio_id){return 0;}
        return $sucursal_id;
    }

    public function StoreUpdateStock($row,$i,$product,$comercio_id,$id_almacen,$columna,$exists,$nombre_sucursal,$sucursal_id){
        
        if($sucursal_id != 0){
        $nombre_columna_s =  $sucursal_id."_stock_".$nombre_sucursal; 
        $nombre_columna_a =  $sucursal_id."_almacen_".$nombre_sucursal; 
        } else {
        ////// STOCK CASA CENTRAL ////
        $nombre_columna_s = 'stock';
        $nombre_columna_a = 'almacen';
        }
        
        $nombre_columna_stock = $columna[$nombre_columna_s];
        $nombre_columna_almacen = $columna[$nombre_columna_a];
        
        $sucursal_id = $this->SetSucursalOrCero($sucursal_id,$comercio_id);
        ...
        $id_almacen = $this->almacenSucursal($pss_stock,$nombre_columna_almacen,$row, $comercio_id);
        ...
        if($nombre_columna_stock != "0"){ 
        $valor_stock = $row[$nombre_columna_stock];
             
              if ($pss_stock != null) {
                $dif = $pss_stock->stock_real - $pss_stock->stock;
                $cantidad_movimiento = (empty($valor_stock) ? 0 : $valor_stock) - $pss_stock->stock_real;
                $this->sd1 = (empty($valor_stock) ? 0 : $valor_stock) - $dif;
              } else {
                $this->sd1 = (empty($valor_stock) ? 0 : $valor_stock);
                $cantidad_movimiento = (empty($valor_stock) ? 0 : $valor_stock);
              }
              ...
              $group = productos_stock_sucursales::updateOrCreate(
                ...
                [
                  'stock' => $this->sd1,
                  'stock_real' => empty($valor_stock) ? 0 : $valor_stock,
                  'referencia_variacion'   => $this->referencia_id,
                  'product_id'  => $product->id,
                  'comercio_id' => $comercio_id,
                  'almacen_id' => $id_almacen
              ...
              $this->HistoricoStock($product->id,$this->referencia_id,$cantidad_movimiento,(empty($valor_stock) ? 0 : $valor_stock),$comercio_id);
    ...
    public function almacenSucursal($pss_stock,$nombre_columna_almacen,$row, $comercio_id)
    {
    
    if($nombre_columna_almacen == "0"){
        if($pss_stock == null){
            $almacen_id = 1;
        } else {
            $almacen_id = $pss_stock->almacen_id;
        }       
    } else {
        $nombre_almacen = $row[$nombre_columna_almacen];
        
        //var_dump($nombre_almacen);
    ...
    public function StoreIVAProducto($row,$i,$product,$comercio_id,$id_almacen,$columna,$exists,$nombre_sucursal,$sucursal_id){
    
    if($sucursal_id == "0"){
    $nombre_columna = 'iva';
    $sucursal_id_buscar = $comercio_id;
    }
    if($sucursal_id != "0"){
    $nombre_columna =  $sucursal_id."_iva_".$nombre_sucursal; 
    $sucursal_id_buscar = $sucursal_id;
    }
    
    
    $nombre_columna_iva = $columna[$nombre_columna];    

    $iva_defecto = $this->ObtenerIVADefecto($sucursal_id_buscar);
    
    $r = $row[$nombre_columna_iva] ?? 0;
    
    // si no se tiene que importar y el prooducto no existe
    if($nombre_columna_iva == "0"){
        $p_iva = productos_ivas::where('sucursal_id',$sucursal_id_buscar)->where('comercio_id',$comercio_id)->where('product_id',$product->id)->first();
        if($p_iva == null){
            
        productos_ivas::create([
          'product_id' => $product->id,
          'comercio_id' => $comercio_id,
          'sucursal_id' => $sucursal_id_buscar,
          'iva' => $iva_defecto
        ]);
    ...
    public function GetPorcentajeDefectoDB($lista_id,$comercio_id){
            ...
            $regla_pasar = $regla ? ($regla->porcentaje_defecto / 100) : 0;
            return $regla_pasar;
    }
    ...
    public function GetReglaPrecioDB($lista_id,$comercio_id){
            ...
            $regla_pasar = $regla ? $regla->regla : 1;
            return $regla_pasar;
    }
    
    public function ActualizarReglaDePreciosDB($product_id,$referencia_variacion,$product){
    ...
    //
    $costo = $this->calcularCostoDespuesDeDescuento($product_id,$referencia_variacion, $costo);
    ...
    public function StoreUpdatePrecioFijo($filasTotales, $product, $comercio_id, $columna, $exists, $nombre_columna_precio, $lista_id, $costo) {
            ...
            if ($nombre_columna_precio == "0") {
                $this->crearPrecioInicial($product, $comercio_id, $lista_id,0);
            }
            
            if ($nombre_columna_precio != "0") {
            
            $precio = $filasTotales[$nombre_columna_precio];
            
            $this->actualizarPrecioListas($lista_id, $nombre_columna_precio, $product, $comercio_id, $precio, $costo, $precio_viejo);
    ...
    // Funciones auxiliares de precios
        ...
        public function getPrecioAnterior($product, $comercio_id, $lista_id) {
            $p_price = productos_lista_precios::where('lista_id', $lista_id)
                ->where('referencia_variacion', $this->referencia_id)
                ->where('comercio_id', $comercio_id)
                ->where('product_id', $product->id)
                ->first();
            
            return $p_price ? $p_price->precio_lista : 0;
        }
        
        public function crearPrecioInicial($product, $comercio_id, $lista_id) {
            $p_price = productos_lista_precios::where('lista_id', $lista_id)
                ->where('referencia_variacion', $this->referencia_id)
                ->where('comercio_id', $comercio_id)
                ->where('product_id', $product->id)
        ...
        public function actualizarPrecioListas($lista_id, $nombre_columna_precio, $product, $comercio_id, $precio, $costo, $precio_viejo) {
 
                $porcentaje_pasar = $this->CambiarPorcentajeReglaPrecioFijo($costo, empty($precio) ? 0 : $precio);
            //    var_dump($porcentaje_pasar);
                ...
                $group = productos_lista_precios::updateOrCreate(
                    ...
                    [
                        'precio_lista' => empty($precio) ? 0 : $precio,
    ...
    public function calcularCostoDespuesDeDescuento($product_id,$referencia_id, $costo_original) {

        $descuentos = productos_descuentos::where('product_id', $product_id)->where('referencia_variacion',$referencia_id)->get();
    ...
    //    var_dump($costo_original,$costo_final,$descuento_final);
        return $costo_final; // Devuelve el precio final después de aplicar todos los descuentos
    }

    
    public function StoreUpdateDescuentos($filasTotales,$i,$product,$comercio_id,$columna,$exists,$referencia_id){
        $LD = listas_descuentos::where('comercio_id', $comercio_id)->get();

        foreach($LD as $ld) {
        $lista = strtolower($ld->nro_descuento);
        $lista_id = $ld->id;
        $nombre_lista = str_replace(' ', '_', $lista);
        
        $this->StoreUpdateDescuentosDB($filasTotales[$i],$i,$product,$comercio_id,$columna,$exists,$nombre_lista,$lista_id,$referencia_id);
        }
    }
}
