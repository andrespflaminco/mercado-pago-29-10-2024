    ...
    try {
      for ($i = 0; $i < count($filasTotales) ; $i++) {
       ...
       if($this->accion == 1){
        ...
        $id_marca       = $this->marcas($filasTotales[$i]['marca'], $comercio_id); //////////// MARCA //////////////  6-6-2024
        ...
        foreach ($filasTotales[$i] as $key => $r) {
          ...
          $this->SetIvaProducto($var,$r,$product,$comercio_id);
...
public function proveedores($nombre_proveedor, $comercio_id)
  ...
  // 6-6-2024
  public function marcas($nombre, $comercio_id)
  {
    ...
    // si tiene otro nombre
    $c = marcas::where('name',$nombre)->where('comercio_id',$comercio_id)->where('eliminado',0)->first();
    // si existe usa ese
  ...
  public function origen($origen)
  {
    ...
    if ($origen == 'ensamblado en la venta') {
      return 3;
    }
...
function obtenerIdEtiqueta($nombre_etiqueta,$comercio_id)
{
    ...
    public function SetIvaProducto($var,$r,$product,$comercio_id){
    
    // Casa central
    
    if (count($var) < 2) {
    if (is_array($var) && $var[0] == "iva") {
