...
class ReportsNuevoController extends Component
{
    ...
    public $nro_comprobante, $comprobante, $canal_venta;
	...
	public function render()
	{
      ...
      //  dd($this->columnaOrden,$this->direccionOrden);
           ...
           if($this->estado_facturacion !== 'all' )
            ...
            ->where('sales.eliminado', $this->estado_filtro);
  ...
  $this->comercio_id = $comercio_id;
	...
	->get();
      ...
      $this->clientes = ClientesMostrador::where('comercio_id', $this->casa_central_id)->where('eliminado',0)->get();
      } else {
      ...
      $this->provincias = provincias::get();
        ...
        return view('livewire.reports-nuevo.component', [
    ...
    public function MetodoPago($value)
    {
      ...
      $this->RenderFactura($this->id_pedido);
       ...
       $this->UpdateIvaGral(0,$nro_venta,$origen);    
        ...
        // Aca tira error desde una sucursal
...
// CORREGIR ESTO PARA CUANDO CAMBIAS DE CLIENTE CON UNA LISTA NUEVA CAMBIE LA LISTA
...
public function RenderFactura($ventaId)
    ...
    /////////////// DETALLE DE VENTA /////////////////////7
       ...
       //
        ...
        foreach($this->total_total as $t) {
            $this->canal_venta = $t->canal_venta;
  ...
  //////////////// PAGOS //////////////
        ...
        ->select('pagos_facturas.deducciones','pagos_facturas.url_comprobante','pagos_facturas.nro_comprobante','mp.nombre as metodo_pago','pagos_facturas.id','pagos_facturas.iva_recargo','pagos_facturas.iva_pago','cajas.nro_caja','pagos_facturas.recargo','pagos_facturas.monto','pagos_facturas.created_at as fecha_pago')
...
// Cambio de estado de los pedidos
    ...
    {   
      ...
      $estado = Sale::select('sales.sucursal_retiro','sales.status','sales.id','users.email','sales.canal_venta','sales.wc_order_id','sales.cliente_id')
      ->join('clientes_mostradors','clientes_mostradors.id','sales.cliente_id')
      ->leftjoin('users','users.cliente_id','clientes_mostradors.id')
      ->where('sales.id', $this->id_pedido)->first();
     ...
     $sucursal_retiro = $estado->sucursal_retiro;
     //dd($sucursal_retiro);
     $items = SaleDetail::where('sale_details.sale_id',$this->id_pedido)->get();
     ...
     foreach ($items as  $item) {
            ...
            // 1-9-2024 29-9-2024
            $produccion_detalle = produccion_detalle::where('sale_details_id',$item->id)->first();
            if($produccion_detalle == null){
                //$this->SwitchUpdateEstado($estado_original,$estado_id,$item,$this->sucursal_id);
                $this->SwitchUpdateEstado($estado_original,$estado_id,$item,$sucursal_retiro);
            } else {
                if($produccion_detalle->tipo_producto != 3){
                //$this->SwitchUpdateEstado($estado_original,$estado_id,$item,$this->sucursal_id);
                $this->SwitchUpdateEstado($estado_original,$estado_id,$item,$sucursal_retiro);
                }
    ...
    {   
      ...
      // Actualizar la produccion
      $this->UpdateQtyInsumosProduccionDetalle($producto_id,$item->quantity);
      //
    ...
    public function SwitchUpdateEstado($estado_original,$estado_id,$item,$sucursal_id) {
            ...
            // Aca si el item es de produccion inmediata no tiene que hacer nada 
            ...
            if(($estado_original == "Entregado")) {
                ...
                // El estado nuevo es cancelado -- OK
                if($estado_id == 4) {
                $return = $this->UpdateSumarStock($item->product_id,$item->referencia_variacion,$item->quantity,$sucursal_id,$estado_id);    // Actualiza stock disponible de casa central -- Sumandole las unidades
                $returnReal = $this->UpdateSumarStockReal($item->product_id,$item->referencia_variacion,$item->quantity,$sucursal_id,$estado_id);    // Actualiza stock real de casa central -- Sumandole las unidades
            ...
            // El estado nuevo es entregado -- OK
                if($estado_id == 3) {
                $return = $this->UpdateRestarStock($item->product_id,$item->referencia_variacion,$item->quantity,$sucursal_id,$estado_id);    // Actualiza stock disponible de casa central -- restandole las unidades
                $returnReal = $this->UpdateRestarStockReal($item->product_id,$item->referencia_variacion,$item->quantity,$sucursal_id,$estado_id);    // Actualiza stock real de casa central -- restandole las unidades
...
public function AbrirModalEditarCliente() {
 ...
 //-------- FUNCION QUE MUESTRA LOS PRODUCTOS PARA AGREGAR UNO NUEVO A LA VENTA --------------------------//
    ...
    ->where('eliminado',0)
    ->where( function($query) {
              $query->where('name', 'like', '%' . $this->query_product . '%')->orWhere('barcode', 'like', $this->query_product . '%')->orWhere('cod_proveedor', 'like', $this->query_product . '%');
    })
            ->limit(5)
            ->get()
            ->toArray();
    ...
    $this->RenderFactura($this->NroVenta);
       ...
       public function selectProduct($item)
       {
         ...
         if($product->tipo_producto == 3){
             $this->emit("msg-error","No se puede agregar el producto de elaboracion inmediata en una venta");
             return;
         }
        ...
        $venta = Sale::find($this->NroVenta);
         ...
         $product_stock = $this->GetProductStock($item,0,$venta->comercio_id); // product_id, referencia_variacion, sucursal_id
         ...
         if($venta->canal_venta == "Venta a sucursales" && $lista_id < 2) {
         $product_price = $product->precio_interno;  
         } else {
         $product_price = $this->GetPrecioProductoById($item,0,$lista_id); // product_id, referencia_variacion, lista_id
         }
        ...
        $datos_facturacion = datos_facturacion::where('datos_facturacions.comercio_id', $venta->comercio_id)->where('predeterminado',1)->first();
        ...
        $detalle_venta = SaleDetail::create([
         ...
         // actualizacion del detalle de compra --
          ...
          detalle_compra_proveedores::create([
            ...
            'comercio_id' => $this->sucursal_id,
            'sale_detail_casa_central' => $detalle_venta->id
          ]);
            
            
            // Si el estado es entregado lo tenemos que sumar al stock real
            if($estado_sale_id == 1){
            $this->UpdateSumarStock($product->id,0,1, $compras_proveedores->comercio_id, 0);	    
            $this->UpdateSumarStockReal($product->id,0,1, $compras_proveedores->comercio_id, 0);
            }

         }
        
        if($estado_sale_id == "1"){
         $this->UpdateRestarStock($product->id,0,1,$venta->comercio_id, 0);
         $stock = $this->UpdateRestarStockReal($product->id,0,1,$venta->comercio_id, 0);
         $usuario_id = Auth::user()->id;    
         $historico_stock = historico_stock::create([
           'tipo_movimiento' => 2,
           'sale_id' => $venta->id,
        ...
        $this->updateQtyPedido($detalle_venta->id, $cantidad_nueva);
         ...
         if($venta->canal_venta == "WooCommerce") {  
         $result = $this->AgregarProductoDetalleWC($product->id,0, $venta->id, 1,$venta->comercio_id,$product_price);
         }
        ...
        // Aca tengo que incluir primero el nuevo registro en produccion
        // $this->UpdateQtyInsumosProduccionDetalle($detalle_venta->id,1);
        //$this->SaveProduccionTrait($origen,$cart,$total,$cantidad_items,$observaciones,$estado,$fecha_produccion,$sale_details_id);
       ...
       public function selectProductOld($item)
       {
        ...
        $venta = Sale::find($this->NroVenta);
         ...
         $product_price = $this->GetPrecioProductoById($item,0,0); // product_id, referencia_variacion, lista_id
         }
   ...
   public function EliminarProductoPedido($id_pedido_prod)
   {
        ...
        //  dd($id_pedido_prod);
          ...
          $this->DeleteProductoInsumosProduccionDetalle($id_pedido_prod,-$this->items->quantity);
         ...
         if($venta->canal_venta == "WooCommerce") {  
         $result = $this->RegistrarReembolsoWC($this->items);
         }
   ...
   public function updateQtyPedido($id_pedido_prod, $cant = 1)
   {
        ...
        $this->UpdateQtyInsumosProduccionDetalle($id_pedido_prod,$cant);
        ...
        //$result = $this->UpdateProductStockIndividualWocommerce($item->product_id,$item->referencia_variacion,$item->comercio_id);
        $result = $this->CambiarProductoDetalleWC($item->product_id,$item->referencia_variacion, $item->sale_id, $cant,$item->comercio_id);
        
        $this->emit('pago-agregado', 'La cantidad fue modificada.');
    }
    
    
    public function DeleteProductoInsumosProduccionDetalle($id_pedido_prod,$cant){
        $produccion_detalles = produccion_detalle::where('sale_details_id',$id_pedido_prod)->first();
        if($produccion_detalles != null){
            $this->updateQtyProduccion($produccion_detalles->id,$cant,2);
        }
    }
    
    public function UpdateQtyInsumosProduccionDetalle($id_pedido_prod,$cant){
        $produccion_detalles = produccion_detalle::where('sale_details_id',$id_pedido_prod)->first();
        if($produccion_detalles != null){
            $this->updateQtyProduccion($produccion_detalles->id,$cant,2);
        }
    ...
    public function UpdateEntregaParcialEstadoInsumosProduccionDetalle($sale_detail_id,$estado_id){
            $produccion_detalle = produccion_detalle::where('sale_details_id',$sale_detail_id)->first();
            if($produccion_detalle != null){
            $this->UpdateEstadoProduccionTrait($estado_id,2,$produccion_detalle->id);
            }
    } 
    
    public function UpdateEstadoInsumosProduccionDetalle($sale_id,$estado_id){
        $sale_details = SaleDetail::where('sale_id',$sale_id)->where('eliminado',0)->get();
    ...
    //dd($product_id,$referencia_variacion,$detalle_venta->comercio_id);
    $product_stock = $this->GetProductStock($product_id,$referencia_variacion,$detalle_venta->comercio_id);
	...
	if( ($stock_anterior < $diferencia_stock) && $product->stock_descubierto == "si" && $product->tipo_producto != 3 ) {
    ...
    $this->UpdateRestarStockReal($product_id,$referencia_variacion,$diferencia_stock, $venta->comercio_id, 0);	    
    $this->UpdateRestarStock($product_id,$referencia_variacion,$diferencia_stock, $venta->comercio_id, 0);
	} else {
    $this->UpdateRestarStock($product_id,$referencia_variacion,$diferencia_stock, $venta->comercio_id, 0);	    
	}
    ...
    $historico_stock =  $this->SetHistoricoStock(3,$this->NroVenta,$product_id,$referencia_variacion,$cantidad_movimiento,$stock,$this->sucursal_id,$detalle_venta->comercio_id);
    ...
    if($detalle_venta->estado == 1) {
    $this->UpdateSumarStock($product_id,$referencia_variacion,$movimiento, $compra->comercio_id, 0);
    $this->UpdateSumarStockReal($product_id,$referencia_variacion,$movimiento, $compra->comercio_id, 0);	    
	}
...
public function SetCasaCentral() {
  ...
  public function CreatePagoCompra($ventaId,$compraId,$pago_casa_central)
  {
    ...
    $monto_total = $pago_casa_central->monto + $pago_casa_central->recargo + $pago_casa_central->iva_pago + $pago_casa_central->iva_recargo;
...
public function SetSucursalPagos($sucursal_id,$pago_id){
    ...
    public function ActualizarTotalesVenta($venta) {
        ...
        $this->details = SaleDetail::where('sale_id',$venta)->where('eliminado',0)->get();
            ...
            $this->ActualizarComisiones($this->venta->id);
...
public function ElegirCUITFacturar($venta_id){
    ...
    $datos = datos_facturacion::where('comercio_id',$this->sucursal_id)->where('eliminado',0)->get();
...
if($ventaIdFactura != "0"){
    ...
    foreach($this->listado_ventas_id as $id){
     $vc = Sale::select('sales.id','sales.nro_factura')->where('sales.id',$id)->first();
     if($vc->nro_factura == null) { 
         $this->FacturarAfip($vc->id,$datos_id);
     }   
    }
}
	...
	{
	    if($ventaIdFactura == null){
	        $ventaIdFactura = $this->NroVenta;
	    }
...
public function SincronizarVentaWC($venta_id,$origen){

$respuesta = $this->SincronizarVentaMostradorEnWC($venta_id);
...
// 27-8-2024
public function ActualizarComisiones($sale_id){

    $sale = Sale::find($sale_id);
    $comision = $sale->alicuota_comision;
    $sale->comision = $comision * $sale->total;
    $sale->save();
    
    $sale_details = SaleDetail::where('sale_id',$sale->id)->get();
    foreach($sale_details as $item){
        $total = ($item->price*$item->quantity) - $item->descuento - ($item->cantidad_promo*$item->descuento_promo) + $item->recargo + $item->iva_total;
        $detail = SaleDetail::find($item->id);
        $detail->comision = $total *  $comision;
        $detail->save();
    }
    
}

public function GetComisionVenta($user_id) {
    $comision = ComisionUsuario::where('user_id', $user_id)->first(); 
