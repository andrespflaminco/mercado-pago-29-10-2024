...
class DashboardNuevo extends Component
{
  ...
  $reportType, $userId, $ventas_por_dia, $dateFrom, $dateTo, $filtro_ingresos_egresos, $saleId, $sucursal_name, $comercio_id, $selected_id, $detalle_ingresos, $Id, $productos, $metodos_pago, $ventas_total, $ventas_cliente, $distances, $seriesData, $mensualizado, $data_total, $data_deuda, $data_mes, $total_mes, $mes,$sucursales_id, $gastos_total_mes, $gastos_mes, $total_gastos, $categorias_total, $categorias_nombre, $total_canal, $canal, $sucursal_id;
  ...
  public $ventas_totales_canal,$promedio_ventas_vendedores,$data_comercio_vendedor;
  ...
  public $selectedVendedoresStock = [];
  public $selectedVendedoresCheckbox = [];
  public $vendedores_elegidos;
  public $data_comercio_name_vendedor;
  
  //------------------------------------//
  
 //16-5-2024
  
 // Filtros grafico ventas por vendedor 
    public $ventas_vendedor, $data_ventas_vendedor, $data_vendedor;
    
 // Filtros de ventas por canal
 public $data_canal,$data_total_canal;

  public $tabla_stock,$tabla_stock_minimo;
  ...
  public $proveedores,$usuarios,$categorias,$objetivo_vendedores;
 ...
 public function mountVendedores($vendedores){
      $usuario = [auth()->user()->id];
      $vendedores_usuario = User::whereIn('comercio_id',$usuario)->where('sucursal',0)->where('cliente_id',null)->pluck('id')->toArray();
      $vendedores_usuario = array_merge($usuario, $vendedores_usuario);

      return $vendedores ? $vendedores : $vendedores_usuario;      
  }
  ...
  public function mount(Request $request)
  {
      ...
      $vendedores = $request->input('vend');
      $this->selectedVendedores =  $this->mountVendedores($vendedores);
  ...
  $this->categorias = Category::where('comercio_id',$this->casa_central_id)->where('eliminado',0)->get();
  ...
  $this->proveedores = proveedores::where('proveedores.comercio_id', $this->casa_central_id)->where('eliminado',0)->get();
  $this->usuarios = User::where('casa_central_user_id', $this->casa_central_id)->get();
  }
...
//    $this->SalesByDate();
    ...
    return view('livewire.dashboard-nuevo.component', [
...
//  array_push($array_sucursales,$this->comercio_id);
  ...
  ->first()->costo;
   ...
   if($this->ventas_totales1 != 0 && $this->ventas_totales1 != null) { $rentabilidad_porcentaje = ($rentabilidad_marginal_venta/$costos_ventas_totales); } else {$rentabilidad_porcentaje = 0;}
  ...
  $this->ingresos_totales1 = pagos_facturas::select(pagos_facturas::raw('(  SUM(pagos_facturas.monto) + SUM(pagos_facturas.recargo) + SUM(pagos_facturas.iva_recargo) + SUM(pagos_facturas.iva_pago) ) as ingresos_totales'));
...
public function IngresosPorMes(){
   ...
   $this->total_ingresos_egresos = pagos_facturas::select(
        pagos_facturas::raw('(IFNULL(SUM(pagos_facturas.monto),0) + IFNULL(SUM(pagos_facturas.recargo) , 0) + IFNULL(SUM(pagos_facturas.iva_recargo) , 0) + IFNULL(SUM(pagos_facturas.iva_pago) , 0) ) as ingresos'),
...
public function VentasPorVendedor(){
    $ventas = Sale::select(
        'users.id',
        'users.name',
    //    Sale::raw("CASE WHEN users.comercio_id = 1 THEN CONCAT(users.name,'-',users.id) ELSE CONCAT(users.name,'-',users.comercio_id) END as name"), // Lógica condicional para comercio_id
        Sale::raw("CASE WHEN users.comercio_id = 1 THEN users.id ELSE users.comercio_id END as comercio_id"), // Lógica condicional para comercio_id
        Sale::raw("CASE WHEN users.comercio_id = 1 THEN users.name ELSE comercio_users.name END as comercio_name"), // Lógica condicional para comercio_id
    ...
    ->join('users as comercio_users', 'comercio_users.id', '=', 'users.comercio_id') // Auto-unión para obtener el nombre del usuario correspondiente al comercio_id
    ->whereIn('sales.comercio_id', $this->selectedSucursales) // $this->selectedSucursales
    ->whereBetween('sales.created_at', [$this->from, $this->to])
    ->where('sales.status', '<>', 'Cancelado')
    ->where('sales.eliminado', 0)
    ->groupBy('users.id', 'users.name', 'users.comercio_id', 'comercio_users.name') // Agrupar por el nombre del usuario del comercio también
    ->orderBy('total_ventas', 'desc') // Ordenar por total_ventas en orden descendente
    ->get();

    
    $this->ventas_vendedor = $ventas;
    $this->data_ventas_vendedor = $ventas->pluck('total_ventas');
    $this->data_comercio_vendedor = $ventas->pluck('comercio_id'); // Asegúrate de pasar el nombre del comercio
    $this->data_comercio_name_vendedor = $ventas->pluck('comercio_name'); // Asegúrate de pasar el nombre del comercio
    ...
    // Emitir los datos incluyendo comercio_name
    if($this->dateFrom !== '' || $this->dateTo !== '' )
    {
        $this->emit('ventas-vendedor', [
            ...
            'data_comercio_vendedor' => $this->data_comercio_vendedor,
            'data_comercio_name_vendedor' => $this->data_comercio_name_vendedor
        ]);
    }                
}
...
public function TablaStockContabilizado(){
    ...
    $select = ['P.barcode','P.name',$costo,$precio,$stock];
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
public function MetodosDePago(){
    ...
    ->select('m.id as id_metodo_pago','m.nombre', 'bancos.nombre as banco','bancos.cbu',pagos_facturas::raw('(SUM(pagos_facturas.monto) + SUM(pagos_facturas.recargo) + IFNULL(SUM(pagos_facturas.iva_recargo) , 0) + IFNULL(SUM(pagos_facturas.iva_pago) , 0)) as total'));
...
public function IngresosEgresos() {
        ...
        if($this->filtro_ingresos_egresos == "Meses") {
          ...
          $this->ingresos_egresos = pagos_facturas::select(
           pagos_facturas::raw('(SUM(pagos_facturas.monto) + SUM(pagos_facturas.recargo) + IFNULL(SUM(pagos_facturas.iva_recargo) , 0) + IFNULL(SUM(pagos_facturas.iva_pago) , 0)) as total_ingresos'),
 ...
 // $this->switch_ventas_unidades = 1;
 // $this->switch_margen_rentabilidad = 2;
   //
  ...
  public function RentabilidadPorCategoria(){
     ...
     // Si es 1 filtra por ventas, si es 2 por unidades vendidas
   //  $this->switch_ventas_unidades_categoria = 1;
    //$this->switch_margen_rentabilidad_categoria = 2;
   //
...
public function AplicarFiltro(){
    $this->AplicarElegirSucursal();
    $this->AplicarElegirVendedor();
}


public function AplicarElegirVendedor(){
    
//    dd($this->selectedVendedoresCheckbox);
    
    // Obtener las claves donde el valor es true
    $clavesTrue = array_keys($this->selectedVendedoresCheckbox, true);
    
    foreach($this->selectedVendedoresCheckbox as $key => $value){
      $this->selectedVendedoresCheckbox[$key] = $value;  
    }
      
    // Reindexar el array
    $this->selectedVendedores = array_values($clavesTrue);
    $selectedVendedoresStock = array_values($clavesTrue);
    
    // Reemplazar 295 con 0 si está presente en el array
    
    
    foreach ($selectedVendedoresStock as $key => $value) {
        if ($value == $this->casa_central_id) {
            $selectedVendedoresStock[$key] = 0;
        }
    }
    
    
    dd($this->selectedVendedores);
    
    $this->selectedVendedoresStock = selectedVendedoresStock;
    $this->vendedores_elegidos = implode(",",$this->selectedSucursales);
}
