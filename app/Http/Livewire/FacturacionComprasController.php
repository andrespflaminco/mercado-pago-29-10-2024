...
class FacturacionComprasController extends Component
{
    ...
    public $razon_social_form, $cuit_form, $condicion_iva_form, $iibb_form,$pto_venta_form,$fecha_inicio_actividades_form,$iva_defecto_form,$relacion_precio_iva_form;
  ...
  public $productos_variaciones_datos = [];
    ...
    public $lista_precios,$proveeedor_selected;
    public $venta_form;
    ...
    public function ProveedorSelected($value)
    {
      $this->proveedor_selected = $value;
      $this->ProveedorSeleccionado = implode(",",$this->proveedor_selected);
    }
    ...
    */
	...
	public function render()
	{
        ...
        if (!Auth::check()) {
            ...
            $this->estado = "display: block;";
                 ...
                 $facturasRepetidas = facturacion::select('sale_id')
                    ...
                    $facturas = compras_proveedores::join('proveedores','proveedores.id','compras_proveedores.proveedor_id')
                    ->select('compras_proveedores.*','proveedores.nombre','proveedores.cuit as cuit_proveedor')
                    ->where('compras_proveedores.comercio_id',$this->sucursal_id)
                    ->where('compras_proveedores.numero_factura','<>',null)
                    ->where('compras_proveedores.numero_factura','<>','')
                    ->get();
                    
                    // Obtener los valores únicos de cuit_proveedor
                    $this->cuit_proveedor = $facturas->pluck('cuit_proveedor')->unique();

                    // Obtener los valores únicos de tipo_comprobante
                    $this->tipo_comprobante = $facturas->pluck('tipo_factura')->unique();
                    
                    // Obtener los valores únicos de condicion_iva
                ...
                $reportes = $this->GetFacturasCompras($from,$to); 
                
                $reportes_totales = $this->GetFacturasComprasTotales($from,$to);    
                
                $this->datos_facturacion = facturacion::where('comercio_id',$this->sucursal_id)->orderBy('id','desc')->get();
                

                   $this->suma_subtotal = $reportes_totales->subtotal ?? 0;
                   $this->suma_totales = $reportes_totales->total ?? 0;
                   $this->iva_total = $reportes_totales->iva ?? 0;
      ...
      ->get();
        ...
        $proveedores = proveedores::where('comercio_id', $this->casa_central_id)
          ->where('eliminado',0)
          ->orderBy('nombre','asc')
          ->get();
        ...
        return view('livewire.facturacion-compras.component', [
        ...
        public function GetFacturasCompras($from,$to){
            $data = compras_proveedores::join('proveedores', 'proveedores.id', '=', 'compras_proveedores.proveedor_id')
                ->select('compras_proveedores.id as factura_id',
                    'compras_proveedores.nro_compra as nro_venta',
                    ...
                    'compras_proveedores.numero_factura as nro_factura',
            ...
            if ($this->proveedor_selected > 0) {
                $data = $data->whereIn('compras_proveedores.proveedor_id', $this->proveedor_selected);
            }
            
            if ($this->tipo_comprobante_buscar != "0") {
                
                $data = $data->where('compras_proveedores.tipo_factura', $this->tipo_comprobante_buscar);
            }
            ...
            ->where('compras_proveedores.numero_factura', '<>', null)
             ->where('compras_proveedores.numero_factura', '<>', "")
                ->groupBy(
                   ...
                   ->where('compras_proveedores.eliminado', 'like', $this->estado_filtro)
        ...
        public function GetFacturasComprasTotales($from,$to){
            $data = compras_proveedores::join('proveedores', 'proveedores.id', '=', 'compras_proveedores.proveedor_id')
                ->select(
                    compras_proveedores::raw('SUM(compras_proveedores.subtotal - compras_proveedores.descuento) as subtotal'),
                    compras_proveedores::raw('SUM(compras_proveedores.iva) as iva'),
                    compras_proveedores::raw('SUM(compras_proveedores.total) as total')
                )
                ->whereBetween('compras_proveedores.created_at', [$from, $to])
                ->where('compras_proveedores.comercio_id', $this->sucursal_id);
...
//////////////////////////////////////////////////////////////////////
        ...
        public function RenderFactura($factura_id){
            ...
            $facturacion = compras_proveedores::find($factura_id);
            $this->proveedor_id = $facturacion->proveedor_id;
            $this->NroFactura = $facturacion->numero_factura;
            $this->tipo_factura = $facturacion->tipo_factura;
            ...
            $this->detalle_venta = detalle_compra_proveedores::where('detalle_compra_proveedores.compra_id', $factura_id)->get();
            
          //  dd($this->detalle_venta);
            
            $this->alicuota_iva = $this->detalle_venta[0]->alicuota_iva;
...
public function ExportarReporteCompras($url) {

    return redirect('facturas-compras/excel/'.$this->sucursal_id.'/'. $url .'/'. Carbon::now()->format('d_m_Y_H_i_s'));
