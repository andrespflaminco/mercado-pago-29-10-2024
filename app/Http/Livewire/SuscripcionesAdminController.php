...
class SuscripcionesAdminController extends Scaner //Component
{
	...
	public function render()
	{
		...
		->select('suscripcions.*','users.name as comercio','planes_suscripcions.nombre as plan_elegido')->orderBy('id','desc')->get();
        
        $this->ChequearEstadoPagoSuscripcionesVencidas();
		return view('livewire.suscripciones-admin.component', [
...
public function GetSuscripcionDatos($suscripcion)
{
    if ($suscripcion !== null) {
        // Aquí se imprime y detiene la ejecución para verificar los datos de $suscripcion
    
        $metodo = 'GET';
        $endPoint = '/v1/preapproval/search?external_reference=' . urlencode($suscripcion);
        $header = [
            'Authorization' => 'Bearer ' . $this->token,
            'Accept' => 'application/json',
        ];

        // Se realiza una solicitud a través de hacerRequest(), asumiendo que esta función está definida correctamente
        $suscripcionResponse = $this->hacerRequest($this->baseUri, $metodo, $endPoint, $header);

        // Devuelve la respuesta de la suscripción
        return $suscripcionResponse;
    } else {
        // Si $suscripcion es nulo, devuelve false
        return false;
    }
}
    ...
    $suscripciones_impagas = Suscripcion::where('proximo_cobro','<',$hoy)->get();
...
//    $suscripcion_datos = $this->GetSuscripcionDatos($pago->external_reference);
//    dd($suscripcion_datos);
    $estado = $pago->status;
    $fechaCarbon = Carbon::parse($pago->date_created);
    $fecha_cobro = $fechaCarbon->format('Y-m-d H:i:s');
   
    $this->registrarPagoIndividual($pago,$suscripcion);

    // Aca tenemos que ver el status de la suscripcion
    ...
    public function registrarPagoIndividual($payment,$suscripcion)
    {      
       ...
       // dd($payment);
			...
			{        
				...
				$SuscripcionCobros->user_id = $suscripcion->user_id;
