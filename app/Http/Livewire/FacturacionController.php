...
class FacturacionController extends Component
{
    ...
    public $provincias,$razon_social_form, $cuit_form, $condicion_iva_form, $iibb_form,$pto_venta_form,$fecha_inicio_actividades_form,$iva_defecto_form,$relacion_precio_iva_form;
    
    public $limite_categoria_actual,$nro_paso, $componentName, $datos_cliente,$condicion_iva_unico,$nombre_cliente_elegido,$cantidad_facturas,$relacion_precio_iva,$alicuota_iva, $cliente_id,$ecommerce_envio_form, $id_cliente_elegido, $caja, $lista_cajas_dia,$monto_inicial, $query_cliente, $estado_facturacion,  $sucursal_stock, $detalle_facturacion, $iva_agregar, $data,  $tipo_pago, $tipos_pago, $details, $sumDetails, $countDetails, $sum, $totales_ver, $cantidad_tickets, $ticket_promedio, $sucursal_id,
    $reportType, $userId, $dateFrom,$cuit_vendedor,$cuit_vendedor_buscar, $dateTo, $rec,$tipo_pago_sucursal, $saleId,$tipo_factura, $codigo_barra_afip, $suma_deuda, $ultimas_cajas, $mail_ingresado, $comercio_id,$status, $codigo_qr, $search, $codigoQR, $clienteId, $selected_id, $suma_totales, $suma_cantidades, $id_pedido, $product_agregar, $estado_estado, $NroVenta, $hr_elegida, $desc, $ventaId, $suma_monto, $suma_cash, $tot, $hojar, $hoja_ruta, $monto, $estado, $estado2, $nombre_hr, $tipo, $fecha_hr, $turno, $observaciones_hr, $dateHojaRuta, $estado_pago, $metodo_pago_sale_detail, $recargo_mp, $nombre_mp, $recargo, $id_pago, $monto_ap, $fecha_ap, $metodo_pago_ap, $fecha_editar,$nro_hoja_elegido, $product_wc, $formato_modal, $total_pago, $recargo_total, $recargo_nvo_venta, $style, $style2, $estado_original, $id_checked, $accion_lote;
    
    public $logo_paso1,$logo_paso2,$logo_paso3;
    public $paso1,$paso2,$paso3;
    public $Nro_Venta, $categoria_monotributo_actual;
  ...
  public $productos_variaciones_datos = [];
    ...
    public $lista_precios,$cliente_selected;
    public $venta_form;
    public $datos_facturacion_existe;
  	public $query_id;
  	public $columns;
  	public $products_s;
    public $mail = [];
    ...
    public $tipo_comprobante_buscar;
    public $cliente_detalle_factura;
    ...
    public $tipo_entrega,$nombre_destinatario,$paso,$limite_categorias_monotributo,$direccion,$ciudad,$nombre_provincia,$telefono;

    public $subtotal_factura,$iva_factura,$total_factura;
    ...
    public function ClienteSelected($ClienteValues)
    {
      $this->cliente_selected = $ClienteValues;
    }

    public function mount()
    {
      	$this->cuit_vendedor_buscar = 0;
        $this->tipo_comprobante_buscar = 0;
        $this->facturas_repetidas = 0;
        ...
        $this->usuario = User::find($comercio_id);
        
        $this->datos_facturacion_elegidos = datos_facturacion::where('comercio_id',$comercio_id)->where('predeterminado',1)->where('eliminado',0)->first();
            
        if($this->datos_facturacion_elegidos == null){
        $this->datos_facturacion_existe = 0;
        } else {
        if($this->datos_facturacion_elegidos->habilitado_afip == 1){
        $this->datos_facturacion_existe = 1;    
        } else {
        $this->datos_facturacion_existe = 2;      
        }
        }
   ...
   /* 
    public function Sincronizar(){
        ...
        foreach($facturacion as $f){
            $fact = facturacion::find($f->id);
            $nro_factura = $fact->nro_factura;
            $nro_factura = explode("-",$nro_factura);
            $tipo_factura = $nro_factura[0];
            
            if($tipo_factura == "FB" || $tipo_factura == "FA"){
            $f->condicion_iva = "IVA Responsable inscripto";
            }
            if($tipo_factura == "FC" || $tipo_factura == "C"){
            $f->condicion_iva = "Monotributo";
            }
            $f->save();
        }
    }
    ...
    {  
            ...
            $this->estado = "display: block;";
                 ...
                 $facturasRepetidas = facturacion::select('sale_id')
                    ...
                    $facturas = facturacion::where('comercio_id',$this->sucursal_id)->get();
                    
                    // Obtener los valores únicos de cuit_vendedor
                    $this->cuit_vendedor = $facturas->pluck('cuit_vendedor')->unique();

                    // Obtener los valores únicos de cuit_vendedor
                    $this->tipo_comprobante = $facturas->pluck('tipo_comprobante')->unique();
                    
                    // Obtener los valores únicos de cuit_vendedor
                    $this->condicion_iva = $facturas->pluck('condicion_iva')->unique();
                    
                    //dd($this->condicion_iva->count());
                    
                    // Verificar si hay un solo valor único
                    if ($this->condicion_iva->count() == 1) {
                    $this->condicion_iva_unico = $this->condicion_iva->first(); // Obtener el valor único
                    } else {
                    $this->condicion_iva_unico = null;    
                    }

                    
                    //dd($this->condicion_iva_unico);
                
                $reportes = facturacion::join('sales','sales.id','facturacions.sale_id')
                   ...
                   ->select('facturacions.cuit_vendedor','facturacions.cae','facturacions.id as factura_id','facturacions.nro_factura','facturacions.nota_credito','facturacions.created_at as fecha_facturacion','sales.created_at','sales.nro_venta','sales.id as sale_id','facturacions.total','facturacions.subtotal','facturacions.iva','u.name as user','m.nombre as nombre_metodo_pago','cm.nombre as nombre_cliente','cm.email','sales.cash','sales.estado_pago','sales.deuda','b.nombre as nombre_banco')
                   ->whereBetween('sales.created_at', [$from, $to])
                   ->where('sales.comercio_id', $this->sucursal_id)
                   ->where('sales.id', 'like',$this->search . '%');
                   
                    if($this->cliente_selected != null) {
                    
                    $reportes = $reportes->whereIn('facturacions.cliente_id', $this->cliente_selected); //  el cliente
        
                    }
                    
                    if($this->cuit_vendedor_buscar != 0) {
                    
                    $reportes = $reportes->where('facturacions.cuit_vendedor', $this->cuit_vendedor_buscar); //  el vendedor
                    ...
                    if($this->facturas_repetidas != 0) {
                    
                    if($this->facturas_repetidas == 1) {
                    $reportes = $reportes->whereIn('facturacions.sale_id', $facturasRepetidas); // Filtra solo aquellos que se repiten
                    }
                    
                    if($this->facturas_repetidas == 2) {
                    $reportes = $reportes->whereIn('facturacions.sale_id', $facturasNoRepetidas); // Filtra solo aquellos que no se repiten
                    }
                    ...
                    $reportes = $reportes->where('facturacions.tipo_comprobante', $this->tipo_comprobante_buscar); // Filtra por tipo de comprobante
                    } else {
                    $reportes = $reportes->whereRaw('facturacions.nota_credito IS NOT NULL'); // Filtra por tipo de comprobante    
                    }
                        
                    }
                    

                   $reportes = $reportes->whereRaw('sales.nro_factura IS NOT NULL')
                   ->groupBy('facturacions.cuit_vendedor','facturacions.cae','facturacions.id','facturacions.nro_factura','facturacions.nota_credito','facturacions.created_at','sales.created_at','sales.nro_venta','sales.id','facturacions.total','facturacions.subtotal','facturacions.iva','u.name','sales.cash','sales.estado_pago','sales.deuda','m.nombre','cm.nombre','cm.email','b.nombre','m.nombre')
                ...
                $this->datos_facturacion = facturacion::where('comercio_id',$this->sucursal_id)->orderBy('id','desc')->get();
                
                $reportes_totales = facturacion::join('sales','sales.id','facturacions.sale_id')
                   ->join('users as u', 'u.id', 'sales.user_id')
                   ->join('metodo_pagos as m','m.id','sales.metodo_pago')
                   ->join('bancos as b','b.id','m.cuenta')
                   ->join('clientes_mostradors as cm','cm.id','sales.cliente_id')
                    ->select(Sale::raw('SUM(facturacions.subtotal) as subtotal'),Sale::raw('SUM(facturacions.iva) as iva'),Sale::raw('SUM(facturacions.total) as total'),Sale::raw('SUM(sales.recargo) as recargo'),Sale::raw('SUM(sales.deuda) as deuda'),Sale::raw('SUM(sales.items) as items'),Sale::raw('COUNT(sales.id) as count_id'))
                    ->whereBetween('sales.created_at', [$from, $to])
                   ->where('sales.comercio_id', $this->sucursal_id)
                   ->where('facturacions.nota_credito', null)
                   ->where('sales.id', 'like',$this->search . '%');
                   
                    if($this->cliente_selected != null) {
                    
                    $reportes_totales = $reportes_totales->whereIn('facturacions.cliente_id', $this->cliente_selected); //  el cliente
        
                    }
                    
                    if($this->cuit_vendedor_buscar != 0) {
                    
                    $reportes_totales = $reportes_totales->where('facturacions.cuit_vendedor', $this->cuit_vendedor_buscar); //  el vendedor
                    ...
                    if($this->facturas_repetidas == 1) {
                    $reportes_totales = $reportes_totales->whereIn('facturacions.sale_id', $facturasRepetidas); // Filtra solo aquellos que se repiten
                    }
                    
                    if($this->facturas_repetidas == 2) {
                    $reportes_totales = $reportes_totales->whereIn('facturacions.sale_id', $facturasNoRepetidas); // Filtra solo aquellos que no se repiten
                    }
                    ...
                    if($this->tipo_comprobante_buscar != "NC") {    
                    $reportes_totales = $reportes_totales->where('facturacions.tipo_comprobante', $this->tipo_comprobante_buscar); // Filtra por tipo de comprobante
                    } else {
                    $reportes_totales = $reportes_totales->whereRaw('facturacions.nota_credito IS NOT NULL'); // Filtra por tipo de comprobante    
                    }
                        
                    } else {
                    $reportes_totales = $reportes_totales->where('facturacions.nota_credito',NULL);    
                    }
                    

                   $reportes_totales = $reportes_totales->whereRaw('sales.nro_factura IS NOT NULL')
                   ->first();                    
                   
                  // dd($this->tipo_comprobante_buscar);

                   $this->suma_subtotal = $reportes_totales->subtotal;
                   $this->suma_totales = $reportes_totales->total;
                   $this->iva_total = $reportes_totales->iva;
      ...
      ->get();
        ...
        $this->provincias = provincias::get();
        
        return view('livewire.facturacion.component', [
          ...
          'clientes' => ClientesMostrador::where('creador_id', $this->sucursal_id)
          ->where('eliminado',0)
          ->orderBy('nombre','asc')
          ->get()
        ])
        ...
        public function VerDetalleFactura($factura_id){
            $this->detalle_facturas = facturacion::where('id',$factura_id)->get();
            $this->cliente_detalle_factura = ClientesMostrador::find($this->detalle_facturas[0]->cliente_id);
            
            $this->emit("ver-factura","");
        }
...
public function RenderFactura($factura_id)
   {
    
    $facturacion = facturacion::find($factura_id);
    $this->cliente_id = $facturacion->cliente_id;
    $this->NroFactura = $facturacion->nro_factura;
    
    $tipo_factura = explode("-",$this->NroFactura);
    $this->tipo_factura = $tipo_factura[0];
    
    // Facturas
    if($this->tipo_factura == "FC") {$this->tipo_factura = "C";}
    if($this->tipo_factura == "FB") {$this->tipo_factura = "B";}
    if($this->tipo_factura == "FA") {$this->tipo_factura = "A";}
    
    $this->detalle_nota_credito = nota_credito::where('factura_id',$factura_id)->first();

    $this->detalle_venta = detalle_facturacion::where('detalle_facturacions.factura_id', $factura_id)->get();
    
    $this->alicuota_iva = $this->detalle_venta[0]->iva;
    
    $this->cliente_id = $facturacion->cliente_id;
    $this->iva_total = $facturacion->iva;
    $this->nota_credito = $facturacion->nota_credito;
    
    $this->subtotal_factura = $facturacion->subtotal;
    $this->iva_factura = $facturacion->iva;
    $this->total_factura = $facturacion->total;
    $this->nro_nota_credito = $facturacion->nota_credito;
...
// Cambio de estado de los pedidos
    ...
    {   
      ...
      $this->HistoricoStock($item->product_id,$item->referencia_variacion,-$item->quantity,$returnReal,$this->sucursal_id,3,1);     
      ...
      $this->HistoricoStock($item->product_id,$item->referencia_variacion,$item->quantity,$returnReal,$this->sucursal_id,1,1);      
      ...
      foreach ($composicion_venta as $detalleVenta) {
          ...
          // Si al menos un elemento tiene estado igual a 0, actualiza la variable $entregado
          $entregado = false;
    ...
    public function SwitchUpdateEstado($estado_original,$estado_id,$item,$sucursal_id) {
            ...
            if(($estado_original == "Pendiente") ||  ($estado_original == "En proceso" ) ||  ($estado_original == "Entrega Parcial" ) ) {
...
public function MailModalVerVenta($factura_id) {
...
public function ExportarReporte($url) {
    ...
    return redirect('facturas/excel/'.$this->sucursal_id.'/'. $url .'/'. Carbon::now()->format('d_m_Y_H_i_s'));
...
public function GetTotalesMonotributo() {
    
                        
    $categorias_monotributo = categorias_monotributo::get();
                  
    $categoriaElegida = null;
                    
    foreach ($categorias_monotributo as $categoria) {
        $minimo = floatval($categoria->minimo);
        $maximo = floatval($categoria->maximo);
                    
        // Verificar si la cantidad está dentro del rango de la categoría
        if ($this->suma_totales >= $minimo && $this->suma_totales < $maximo) {
            $categoriaElegida = $categoria->categoria;
            break; // Salir del bucle una vez que se encuentra la categoría
            }
        }
                    
        if ($categoriaElegida !== null) {
            $this->categoria_monotributo_actual = $categoriaElegida;
            $this->limite_categoria_actual = categorias_monotributo::select("maximo")->where("categoria",$this->categoria_monotributo_actual)->orderBy("maximo","desc")->first()->maximo;
            } else {
            $this->categoria_monotributo_actual = null;
            $this->limite_categoria_actual = null;
            }
        
         
        $this->limite_categorias_monotributo = categorias_monotributo::select("maximo")->orderBy("maximo","desc")->first()->maximo;
}

 
 public function StoreDatosFacturacion(){
    ...
    $data["body"] = "Solicitud de incorporacion de AFIP del usuario ".Auth::user()->name." , con email: ".Auth::user()->email.". CUIT: ".$this->cuit_form;
  ...
  $datos_facturacion = datos_facturacion::create([
     'razon_social' => $this->razon_social_form,
     'domicilio_fiscal' => null,
     'localidad' => null,
     'provincia' => null,
     'iva_defecto' => $this->iva_defecto_form,
     'pto_venta' => $this->pto_venta_form,
     'relacion_precio_iva' => $this->relacion_precio_iva_form,
     'fecha_inicio_actividades' => $this->fecha_inicio_actividades_form,
     'condicion_iva' => $this->condicion_iva_form,
     'iibb' => $this->iibb_form,
     'cuit' => $this->cuit_form,
     'comercio_id' => $comercio_id,
     'habilitado_afip' => 0
