...
class EditarRecetasController extends Component
{
  ...
  public function render()
  {
    ...
    ->where('products.id', 'like', $this->producto_id)
    ->where('r.referencia_variacion', 'like', $this->referencia_variacion)
     ...
     foreach ($products as $p)
  {
    ...
    return view('livewire.componentes_editar.component',[
      'data' => $products
    ])
   ...
   foreach ($items as $i)
{
   ...
   return back();
    ...
    public function removeProductFromCart($insumo) {
        // dd($insumo);
        $cart_recetas = new CartRecetas;
         $cart_recetas->removeProduct($insumo);
  ...
  public function saveSale()
  {
    ...
    DB::beginTransaction();
      ...
      $cost = receta::where('product_id',$this->producto_id)
		...
		if($this->referencia_variacion != 0) {
		$update_product = productos_variaciones_datos::where('product_id',$this->producto_id)->where('referencia_variacion',$this->referencia_variacion)->orderBy('id','desc')->first();
		$update_product->cost = $cost->cost/$cost->rinde;
		$update_product->save();
		} else {
		 $update_product = Product::find($this->producto_id);
		 $update_product->cost = $cost->cost/$cost->rinde;
		 $update_product->save();
		}
