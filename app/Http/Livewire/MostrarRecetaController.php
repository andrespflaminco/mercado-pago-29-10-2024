...
class MostrarRecetaController extends Component
{
  use WithPagination;
  use WithFileUploads;
  ...
  public function render()
  {
    ...
    $this->nombre = Product::leftjoin('productos_variaciones_datos','productos_variaciones_datos.product_id','products.id')
    ->where('products.id',$this->producto_id )
    ->where('productos_variaciones_datos.referencia_variacion', $this->referencia_variacion)
   ...
   // dd($products);
    ...
    $suma = $products->sum(function($item){
              return $item->cost*$item->relacion_medida*$item->cantidad;
    });
    
    $rinde = $products->sum(function($item){
              return $item->rinde;
    });
    ...
    return view('livewire.mostrar_receta.component',[
      'data' => $products,
      'rinde' => $promedio,
      'suma' => $suma
    ])
