...
class CategoriesController extends Component
{
	...
	public function render()
	{
		...
		if(strlen($this->search) > 0)
			...
			$data = Category::where('comercio_id', 'like', $this->casa_central_id)
			->where('eliminado',$this->estado_filtro)
			->orderBy('id','desc')
			->paginate($this->pagination);
	...
	public function Store()
	{
		...
		$rules = [
			'name' => ['required',Rule::unique('categories')->where('comercio_id',$comercio_id)->where('eliminado',0)],
		];
	...
	public function Update()
	{
		...
		$rules =[
			'name' => ['required',Rule::unique('categories')->ignore($this->selected_id)->where('comercio_id',$comercio_id)->where('eliminado',0)],
		];
	...
	public function Destroy(Category $category)
	{
		...
		$this->resetUI();
		$this->emit('category-updated', 'Categoría Eliminada');
	...
	public function RestaurarCategoria(Category $categoria)
	{
		$categoria->update([
			'eliminado' => 0
		]);
    ...
    $gastos_checked = Category::select('categories.id','categories.comercio_id')->whereIn('categories.id',$ids)->get();
