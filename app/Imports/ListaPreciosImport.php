...
class ListaPreciosImport implements OnEachRow, SkipsEmptyRows, WithHeadingRow,  WithValidation, SkipsOnError, WithBatchInserts, WithChunkReading, WithCalculatedFormulas, WithMultipleSheets
{
    use SkipsErrors;
   ...
   public function onRow(Row $row)
    {
        ...
        $casa_central_id = $user->casa_central_user_id;
        
        $casa_central_id = $comercio_id;
        
        //////////// PRECIO //////////////

        $pre = $row['precio_final'];

        if($pre == '') {
          $prec = 0;
        }
        else {
          $prec = $row['precio_final'];
        }
        
        /////////// INTERNO //////////////

        $precio_interno = $row['precio_sucursales'];
        ...
        if($cod_variacion == '') {
          ...
          if($product_cost != null) {
            ...
            $product_cost->update([
            'cost' => $row['costo'] ?? 0,
            'precio_interno' => $precio_interno ?? 0
            	]);
          }
