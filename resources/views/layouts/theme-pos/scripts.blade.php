...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelector(".chat-bot").addEventListener("mouseenter", function() {
        var bubble = document.querySelector(".chatbot-bubble");
        bubble.style.display = "block";
        clearTimeout(hideBubbleTimeout); // Cancelar el temporizador de ocultar la burbuja del chat si estè´° activo
    });

    // Agregar evento de mouseleave para ocultar la burbuja del chat despuè´±s de 2 segundos
        ...
        hideBubbleTimeout = setTimeout(function() {
         ...
         <script>
            document.addEventListener("livewire:before-sending", function(event) {
                // Verificar si el usuario estè´° autenticado
                // Si no estè´° autenticado, redirigir al usuario al inicio de sesiè´—n
                if (!window.Laravel || !window.Laravel.authenticated) {
                    window.location.href = "{{ route('login') }}";
                    event.preventDefault(); // Evitar que Livewire envè´øe la solicitud
                }
            });
        </script>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.livewire.on('msg-success', msg => {
        swal({
