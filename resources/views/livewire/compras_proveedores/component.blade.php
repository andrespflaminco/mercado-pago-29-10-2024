@inject('cart', 'App\Services\Cart')
    ...
    <div class="page-header">
		...
		<div class="col-sm-12 col-md-12">
            ...
            <label>Sucursal que compra:</label>
			<div class="input-group mb-0">
			    
				<div class="input-group-prepend">
					<span wire:click="BuscarCode($('#code').val())" class="input-group-text input-gp">
						<i style="padding: 0.30rem !important; font-size:16px !important;" class="fas fa-user"></i>
						</span> 
				</div>
				<select class="form-control" wire:model="sucursal_elegida">
				    <option value="{{auth()->user()->id}}">{{auth()->user()->name}}</option>
				    @foreach($sucursales as $s)
				    <option value="{{$s->sucursal_id}}">{{$s->nombre}}</option>
				    @endforeach
				</select>
			</div>
			</div>
