<div>
    ...
    @if($ver == 0)
					...
					<div class="page-title">
							...
							<h6>Ver listado de movimientos de stock</h6>
						</div>
						<div class="page-btn">               											    
                			
                			@if(Auth::user()->sucursal != 1)
                			@if(Auth::user()->profile != "Cajero" )
							<a href="{{ url('movimiento-stock')}}" class="btn btn-added"><img src="{{ asset('assets/pos/img/icons/plus.svg') }}"  alt="img" class="me-1">Nuevo movimiento</a>
						    @endif
						    @endif
									...
									<tbody>
									   	@foreach($data as $m)
													<tr>
														...
														<td>
															{{number_format($m->items,0,",",".") }}
														</td>
														<td>
														    <a class="me-2" href="remito-movimiento-stock/pdf/{{$m->id}}" target="_blank">
														        <img src="{{ asset('assets/pos/img/icons/printer.svg') }}" alt="img">
            												</a>
														    <a class="me-2" href="javascript:void(0)" wire:click.prevent="RenderFactura({{$m->id}})">
					...
					@if($ver == 1)
					@include('livewire.movimiento-stock-resumen.ver-movimiento')
					@endif
					<!-- /product list -->
