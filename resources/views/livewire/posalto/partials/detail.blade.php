				...
				<tbody>
					...
					<tr>
						...
						<td class="text-center">
							<div style="display:flex;">
								<h6 style="margin-bottom: 0; margin-top:2px; "></h6>
									<input class="boton-precio" type="number" id="al{{$item->id}}"
									wire:change="updateAlt({{$item->id}}, $('#al' + {{$item->id}}).val() )"
									value="{{$item->attributes['alto']}}">
							</div>
						...
						<td class="text-center">
							<div style="display:flex;">
								<h6 style="margin-bottom: 0; margin-top:2px; "></h6>
									<input class="boton-precio" type="number" id="an{{$item->id}}"
									wire:change="updateAn({{$item->id}}, $('#an' + {{$item->id}}).val() )"
									value="{{$item->attributes['ancho']}}">
							</div>

						</td>
						<td class="text-center">
							<h6>
								${{number_format($item->price * $recargo * $item->quantity * $item->attributes['ancho'] * $item->attributes['alto'],2)}}
							</h6>
						</td>
