...
<div class="page-header">
						...
						<div class="page-title">
						    ...
						    @if($forma_edit != 1)
							<h4>{{ $selected_id > 0 ? 'Editar' : 'Agregar' }} @if($es_insumo == false) Producto @else Insumo @endif</h4>
							<h6>{{ $selected_id > 0 ? 'Modificar el ' : 'Crear un nuevo ' }} @if($es_insumo == false) Producto @else Insumo @endif </h6>
							@else
						...
						<div class="card-body">
							<div class="row">
								...
								<div class="col-lg-9 col-sm-6 col-12">
									<div class="form-group">
										<label>Nombre del @if($es_insumo == false) producto @else insumo @endif </label>
								...
								<div class="col-lg-3 col-sm-6 col-12">
									<div class="form-group">
										...
										<select wire:model='proveedor' {{ $es_sucursal == 1? 'disabled' : '' }} {{ $forma_edit == 1? 'disabled' : '' }} {{1 < $tipo_producto? 'disabled' : '' }}  wire:change.defer='ModalProveedor($event.target.value)'  class="form-control">
								...
								<div class="col-lg-3 col-sm-6 col-12">
									<div class="form-group">
										...
										<select wire:model='categoryid' {{ $es_sucursal == 1? 'disabled' : '' }} {{ $forma_edit == 1? 'disabled' : '' }} wire:change.defer='ModalCategoria($event.target.value)' class="form-control">
									        <option value="Elegir" disabled >Elegir</option>
								...
								<div class="col-lg-3 col-sm-6 col-12">
									<div class="form-group">
										...
										<select wire:model='subcategoria_id' {{ $es_sucursal == 1? 'disabled' : '' }} {{ $forma_edit == 1? 'disabled' : '' }}  class="form-control">
									        <option value="1" selected >Sin subcategoria</option>
                                            @foreach($subcategorias as $c)
                                            <option value="{{$c->id}}">{{$c->nombre}}</option>
                                            @endforeach
                                             <option hidden value="AGREGAR" style="padding:20px !important; " class="btn btn-dark">+ NUEVA SUBCATEGORIA</option>
								...
								<div class="col-lg-3 col-sm-6 col-12">
									<div class="form-group">
										<label>Marcas</label>
										<select wire:model='marca_id' {{ $es_sucursal == 1? 'disabled' : '' }} {{ $forma_edit == 1? 'disabled' : '' }} wire:change.defer='ModalMarca($event.target.value)' class="form-control">
								...
								<div class="col-lg-3 col-sm-6 col-12">
									...
									<select class="form-control" id="producto_tipo" wire:model="producto_tipo" {{$selected_id > 0 ? 'disabled' : '' }}  {{ $es_sucursal == 1? 'disabled' : '' }} {{ $forma_edit == 1? 'disabled' : '' }} wire:change="ProductoTipo()">
                                     <option value="Elegir">Elegir</option>
                                     @if($es_insumo == false)
                                     <option value="s">Producto Simple</option>
                                     <option value="v">Producto Variable</option>
                                     @else
                                     <option value="s">Insumo Simple</option>
                                     @endif
                                    </select>
								...
								@include('livewire.products-nuevo.ficha-producto.unidades-medida')
								
								@include('livewire.products-nuevo.ficha-producto.iva')
								...
								@if($producto_tipo == "s")
                                	...
                                	@include('livewire.products-nuevo.ficha-producto.producto-simple')
                            	...
                            	@include('livewire.products-nuevo.ficha-producto.producto-variable')
                            	
                                @endif
                                <!---- / Si es un producto variable ----->
		
		                        @include('livewire.products-nuevo.ficha-producto.input-imagenes')
								...
								<div class="col-lg-12">
								    ...
								    @if($selected_id < 1)
								     <a  onclick="GuardarDatos()"   href="javascript:void(0);" class="btn btn-submit me-2">GUARDAR</a>
								     @else
								     <a  onclick="GuardarDatos()"   href="javascript:void(0);" class="btn btn-submit me-2">ACTUALIZAR</a>
                                     @endif
