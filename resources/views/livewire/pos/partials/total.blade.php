		...
		<div>
			...
			<!---	<h5 class="text-center mb-3">RESUMEN DE VENTA </h5> -->
				<div class="connect-sorting-content">
				    ...
				    @include('livewire.pos.partials.puntos_venta')
				    
					<div class="card simple-title-task ui-sortable-handle">
					    <div style="margin-left: 15px; margin-right: 15px;">
					        @include('livewire.pos.partials.tipo-factura') 
					    </div>
					     ...
					     @if( $cliente != null)
                            ...
                            @if($cliente->monto_maximo_cuenta_corriente != null)
                            <div style="border: solid 1px;   border-color: #eee;" class="card-body">
                            <div  class="task-header text-center">
                            <h4>$ {{ number_format($maximo_cta_cte - $saldo_cta_cte,2,",",".") }}</h4>
                            <h6>Saldo disponible </h6>
                            <h6>en cuenta corriente</h6>
						  ...
						  <div style="width:100%;" class="btn-group  mb-4 mr-2">
                                    ...
                                    <button style="font-size:14px !important;" class="btn btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                     ...
                                     <div class="dropdown-divider"></div>
                                        ...
                                        <a class="dropdown-item" href="javascript:void(0);" wire:click.prevent="ElegirCaja({{$uc->id}})">Caja # {{$uc->nro_caja}} ( {{\Carbon\Carbon::parse($uc->created_at)->format('d/m/Y')}} ) - {{$uc->nombre_usuario}} </a>
                                        @else
                                        <a class="dropdown-item" style="background: #eee !important;" href="javascript:void(0);" wire:click.prevent="NoSePuedeElegirCaja()">Caja # {{$uc->nro_caja}} ( {{\Carbon\Carbon::parse($uc->created_at)->format('d/m/Y')}} ) - {{$uc->nombre_usuario}} </a>
                                        @endif 
						...
						<div>
                        	...
                        	<button wire:click="selectEstado()" type="button" style=" width: 100%; margin-top: 10px; margin-bottom: 10px; !important; margin-right: 15px;" class="btn btn-dark" > Pendiente de Retiro en sucursal </button>
                        	@endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-7" style="font-size:12px !important;">+ Recargo ( {{round($recargo*100,2)}}%  )</div>
                        ...
                        @if( $metodo_pago != '3' && ( 0 <= (floatval($efectivo ?? 0) + 0.01 - floatval($total ?? 0)) ) )
						<button style="font-size: 14px;" wire:click.prevent="saveSale" wire:loading.attr="disabled" {{0 < $itemsQuantity? '' : 'disabled' }} class="btn btn-dark btn-md btn-block">
						<span wire:loading.remove>GUARDAR</span>
						<span wire:loading>GUARDANDO...</span>	
						</button>
