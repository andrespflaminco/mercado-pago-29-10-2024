<div >	                
					...
					<div class="page-title">
							<h4>Almacenes</h4>
							<h6>Ver listado de almacenes</h6>
						</div>
						<div class="page-btn">
						    @if(Auth::user()->sucursal != 1)
						    @if(Auth::user()->profile != "Cajero" )
							<a href="javascript:void(0)" wire:click="Agregar()" class="btn btn-added"><img src="{{ asset('assets/pos/img/icons/plus.svg') }}"  alt="img" class="me-1">Agregar almacen</a>
						    @endif
						    @endif
						</div>
					</div>
								...
								<table class="table">
									...
									<tbody>
										...
										<tr>
											<td hidden>
												<label class="checkboxs">
												    <input type="checkbox" wire:model.defer="id_check" tu-attr-id="{{($almacen->id)}}"  class="mis-checkboxes" value="{{$almacen->id}}">
													<span class="checkmarks"></span>
												</label>
											</td>
											<td>
											...
											<td>
											    @if(Auth::user()->sucursal != 1)
											    @if(Auth::user()->profile != "Cajero" )
												<a class="me-3" href="javascript:void(0)" wire:click.prevent="Edit({{$almacen->id}})" >
					...
					@if($agregar == 1)
					@include('livewire.seccionalmacens.agregar-editar-almacen')
					@endif 
