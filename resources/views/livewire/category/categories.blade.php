<div >	                
					...
					<div class="page-title">
							<h4>Categorias</h4>
							<h6>Ver listado de categorias</h6>
						</div>
						<div class="page-btn">               											    
                			
                			@if(Auth::user()->sucursal != 1)
                			@if(Auth::user()->profile != "Cajero" )
							<a href="javascript:void(0)" wire:click="Agregar()" class="btn btn-added"><img src="{{ asset('assets/pos/img/icons/plus.svg') }}"  alt="img" class="me-1">Agregar categoria</a>
						    @endif
						    @endif
								...
								<table class="table">
									...
									<tbody>
										...
										<tr>
											...
											<td >
												<a href="javascript:void(0);" wire:click.prevent="Edit({{$category->id}})">{{$category->name}}</a>
											</td>
											...
											<td>
											    ...
											    @if(Auth::user()->profile != "Cajero" )
											    @if($estado_filtro == 0 )
												<a class="me-3" href="javascript:void(0)" wire:click.prevent="Edit({{$category->id}})" >
												...
												@else
            								    <a href="javascript:void(0)" onclick="RestaurarCategoria('{{$category->id}}')" class="btn btn-light" title="Restaurar">
            										RESTAURAR
            									</a>
					...
					@if($agregar == 1)
					@include('livewire.category.agregar-editar-categoria')
					@endif 
