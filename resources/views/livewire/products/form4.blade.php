...
<div class="row">
    ...
    <label for="file-input">
        ...
        <img height="230" width="240" src="{{ $base64 }}" id="image-upload" class="rounded">
    </label>

    <input hidden id="file-input" type="file"   accept="image/x-png, image/gif, image/jpeg" />
                   ...
                   @if(auth()->user()->sucursal != 1)
                    ...
                    <div class="form-group col-4">
                     <label>{{auth()->user()->name}}</label>
                       <input type="text" wire:model.lazy="stock_sucursal.{{$variaciones['referencia_id']}}|0|{{$variaciones['var_nombre'] ?? 0}}|{{$variaciones['var_id'] ?? 0}}" class="form-control" placeholder="ej: 0.00" >
                     @error('stock_sucursal') <span class="text-danger er">{{ $message }}</span> @enderror
                    </div>
