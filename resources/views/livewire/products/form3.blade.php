   ...
   @if($lista_precios == null)
    ...
    <div class="form-group col-6">
        <label>Categoría</label>
          <select wire:model.defer='categoryid' wire:change='ModalCategoria($event.target.value)' class="form-control">
            <option value="Elegir" disabled >Elegir</option>
...
<div class="col-sm-12 col-md-4">
 <div class="form-group">
  <label>Seccion del almacen</label>
    <select wire:model.defer='almacen' wire:change='ModalAlmacen($event.target.value)' class="form-control">
      <option value="Elegir" disabled >Elegir</option>
                   ...
                   @if(auth()->user()->sucursal != 1)
                    ...
                    <div class="form-group col-4">
                     <label>{{auth()->user()->name}}</label>
                       <input type="text" wire:model.lazy="stock_sucursal.{{$variaciones['referencia_id']}}|0|{{$variaciones['var_nombre'] ?? 0}}" class="form-control" placeholder="ej: 0.00" >
                     @error('stock_sucursal') <span class="text-danger er">{{ $message }}</span> @enderror
                    </div>
                    ...
                    @foreach ($sucursales as $llave => $sucu)
                              ...
                              <input type="text" class="form-control" wire:model="stock_sucursal.{{$variaciones['referencia_id']}}|{{ $sucu->sucursal_id }}|{{$variaciones['var_nombre'] ?? 0}}" />
                            </div>
