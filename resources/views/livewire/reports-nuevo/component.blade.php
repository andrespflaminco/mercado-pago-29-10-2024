<div>	                
                  ...
                  @include('livewire.reports-nuevo.configurar-columnas')
							...
							<div class="table-responsive">
								<table class="table">
									<thead>
                                        ...
                                        <th wire:click="OrdenarColumna('dias_desde_creacion')" @if(!$columns['dias_desde_creacion']) style="display: none;" @endif>DIAS DE DEUDA
                                    ...
                                    <tr>
                                        ...
                                        <a href="javascript:void(0)" wire:click.prevent="RenderFactura({{$d->id}})" >
                                        {{$d->nro_venta}}    
                                        </a>
                                        @if($wc != null)
                                        @if($d->wc_order_id)
                                        <a target="_blank" title="Ver en woocommerce"  href="{{$wc->url . 'wp-admin/admin.php?page=wc-orders&action=edit&id=' . $d->wc_order_id }}"><i style="margin-right:4px;" class="fab fa-wordpress-simple"></i></a>
    	                                @else
    	                                <a href="javascript:void(0)" title="Sincronizar en woocommerce" wire:click.prevent="SincronizarVentaWC({{$d->id}},0)" >
                                        ...
                                        <td @if(!$columns['dias_desde_creacion']) style="display: none;" @endif>
                                        @if(0 < $d->deuda)
                                        @if($d->plazo_cuenta_corriente < $d->dias_desde_creacion)
                                        <span class="badges bg-lightred">IMPAGO: {{$d->dias_desde_creacion}} DIAS</span>
                                        @else
                                        <span class="badges bg-lightyellow" style="background: #8ea0af !important;">IMPAGO: {{$d->dias_desde_creacion}} DIAS</span>
                                        @endif
                                        @endif
                                        </td>
                                              ...
                                              <div class="btn-group mb-1 mr-1">
                                                    ...
                                                    @if(Auth::user()->plan != 1)
                                                     ...
                                                     @if($d->nro_factura > 0)
                                                      ...
                                                      <a hidden href="javascript:void(0);" class="dropdown-item" wire:click="ElegirCondicionesIVA('{{$d->id}}')"><i class="flaticon-home-fill-1 mr-1"></i>Facturar</a>
                                                      
                                                      @else
                                                      
                                                      @if($d->nro_factura == null)
                                                      <a hidden href="javascript:void(0);" class="dropdown-item" onclick="ConfirmFactura('{{$d->id}}')"><i class="flaticon-home-fill-1 mr-1"></i>Facturar</a>
                                                      @endif
                                                      @endif
                                                      @endif
                                                      @endif
	...
	@else
	@include('livewire.reports-nuevo.ver-venta')
	@endif
...
function ConfirmPago(id) {
  ...
  function ConfirmCambiarEstado(producto_id,accion) {

    swal({
      title: 'CONFIRMAR',
      text: '¿QUIERE CAMBIAR EL ESTADO DE ENTREGA?',
      showCancelButton: true,
      cancelButtonText: 'Cancelar',
      cancelButtonColor: '#fff',
      confirmButtonColor: '#3B3F5C',
      ...
      if (result.value) {
        window.livewire.emit('EntregaParcial', producto_id,accion)
        swal.close()
      }
