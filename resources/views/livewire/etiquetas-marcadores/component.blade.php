<div >	                
					...
					<div class="page-title">
							<h4>Etiquetas</h4>
							<h6>Ver listado de etiquetas</h6>
						</div>
						<div class="page-btn">               											    
                			
                			@if(Auth::user()->sucursal != 1)
                			@if(Auth::user()->profile != "Cajero" )
							<a href="javascript:void(0)" wire:click="Agregar()" class="btn btn-added"><img src="{{ asset('assets/pos/img/icons/plus.svg') }}"  alt="img" class="me-1">Agregar etiqueta</a>
						    @endif
						    @endif
								...
								<table class="table">
									<thead>
										<tr>
											...
											<th>Categoria de le etiqueta</th>
											<th>Acciones</th>
										</tr>
									</thead>
									<tbody>
										...
										<tr>
											<td>
											    ...
											    @if(Auth::user()->profile != "Cajero" )
												<label class="checkboxs">
												    <input type="checkbox" wire:model.defer="id_check" tu-attr-id="{{($etiqueta->id)}}"  class="mis-checkboxes" value="{{$etiqueta->id}}">
													<span class="checkmarks"></span>
												</label>
												@endif
												@endif
											...
											<td >
												<a href="javascript:void(0);" wire:click.prevent="Edit({{$etiqueta->id}})">{{$etiqueta->nombre}}</a>
											</td>

											<td >
												<a href="javascript:void(0);" wire:click.prevent="Edit({{$etiqueta->id}})">{{$etiqueta->origen}}</a>
											</td>
											...
											<td>
											    ...
											    @if(Auth::user()->profile != "Cajero" )
											    @if($estado_filtro == 0 )
												<a class="me-3" href="javascript:void(0)" wire:click.prevent="Edit({{$etiqueta->id}})" >
													<img src="{{ asset('assets/pos/img/icons/edit.svg') }}"  alt="img">
												</a>
												<a href="javascript:void(0)" onclick="Confirm('{{$etiqueta->id}}')"  >
													<img src="{{ asset('assets/pos/img/icons/delete.svg') }}" alt="img">
												</a>
												@else
            								    <a href="javascript:void(0)" onclick="RestaurarCategoria('{{$etiqueta->id}}')" class="btn btn-light" title="Restaurar">
            										RESTAURAR
            									</a>
					...
					@if($agregar == 1)
					@include('livewire.etiquetas-marcadores.agregar-editar')
					@endif 
...
<script type="text/javascript">
    ...
    swal({
      title: 'CONFIRMAR',
      text: 'QUIERE RESTAURAR EL REGISTRO?',
      showCancelButton: true,
