			...
			<div class="widget-heading">
				...
				<ul class="tabs tab-pills {{ $sucursal_id != $comercio_id ? 'hide-sucursales' : '' }}">
					...
					<li>
						...
						<a hidden href="javascript:void(0)" data-toggle="modal" data-target="#ExportarLista" class="tabmenu bg-dark mr-3">Exportar Precios</a>
						<a hidden href="{{url('import-precios')}}" class="tabmenu bg-dark mr-3 {{ Auth::user()->sucursal == 1 ? 'hide-sucursales' : '' }}">Importar Precios</a>
							 ...
							 <input type="checkbox" wire:model.defer="id_check" tu-attr-id="{{($product->id)}}"  class="mis-checkboxes" value="{{$product->id}}">
							    ...
							    text-align: center;" class="input-group mb-4">
                                   ...
                                   <!---- Si el producto es variable ----->
                                    ...
                                    @if($pl->lista_id == 0 && $pl->eliminado == 0)
								...
								@foreach($lista_precios as $list)
                                    ...
                                    @if($pl->lista_id == $list->id && $pl->eliminado == 0)
