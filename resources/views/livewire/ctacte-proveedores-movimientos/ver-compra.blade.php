						...
						<div class="card-body">
							<div class="row">
									...
									<div class="form-group">
										...
										<div class="row">
											<div class="col-lg-10 col-sm-10 col-10">
												<select disabled {{$proveedor_id == 2 ? 'disabled' : '' }} wire:model="proveedor_id" wire:change="CambiarProveedor" class="form-control">
								...
								<div class="form-group">
										...
										<div class="row">
											<div class="col-lg-10 col-sm-10 col-10">
											<input readonly {{ (auth()->user()->sucursal == 1 && $proveedor_id == 2) ? 'disabled' : '' }} type="date" wire:model="fecha_compra" wire:change="CambiarFecha" class="form-control">
											</div>
										</div>
									</div>
								</div>
								...
								<a class="btn btn-light" wire:click="ExportarCompra({{$id_compra}})">Exportar compra en Excel</a>
								</div>
								</div>

								<div class="col-lg-12 col-sm-6 col-12"></div>
							</div>
                        	<div class="row">
									...
									<table class="table">
										...
										<tbody>
										    ...
										    <tr>
												...
												<td>
                                                <input  readonly type="number" type="number" class="boton-editar"  {{$proveedor_id == 2 ? 'disabled' : '' }} value="{{number_format($dc->cantidad,0)}}" id="qty{{$dc->id}}"
                                                wire:change="updateQty('{{$dc->id}}|{{$dc->referencia_variacion}}', $('#qty' + {{$dc->id}}).val() )" min="1" onchange="Update({{$dc->id}});" >
                                                </td>
												<td>
												<div>
                                                <input readonly type="number" class="boton-editar"  {{$proveedor_id == 2 ? 'disabled' : '' }} value="{{$dc->precio }}" id="price{{$dc->id}}"
							...
							<div class="row">
								<div class="col-lg-12 float-md-right">
									<div class="total-order">
										...
										<ul>
											...
											<li class="total">
												<h4>Pago</h4>
												<h5>$ {{number_format($t->total-$t->deuda,2)}}</h5>
											</li>
											<li class="total">
												<h4>Deuda</h4>
								...
								<div class="table-responsive">
                                       ...
                                       @if(0 <= $t->deuda)
                                       <a href="javascript:void(0);" wire:click.prevent="AgregarPago({{$id_compra}})">Agregar pago </a>
                                       @else
                                       <a href="javascript:void(0);" onclick="MensajeAgregarPago({{$id_compra}})">Agregar pago </a>
                                       @endif
                                       </div>
                                       @endif
                                       @endforeach
