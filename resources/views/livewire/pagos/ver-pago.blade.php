          ...
          <div class="modal-content">
              ...
              <div class="row">
                 ...
                 <div style="margin-bottom: 0 !important;" class="input-group mb-12">
                   ...
                   <input {{$formato_ver == 0 ? 'readonly' : '' }} autocomplete="off" type="number" id="title" wire:model.lazy="monto_ap" wire:keydown.enter='MontoPagoEditarPagoForm($event.target.value,"{{$tipo_pago_seleccionado}}")' wire:change='MontoPagoEditarPagoForm($event.target.value,"{{$tipo_pago_seleccionado}}")' class="form-control" required="">
               ...
               <br>
                 <div class="col-sm-12 col-md-12">
                   ...
                   <div class="form-group">
                    <label> Tipo de @if($tipo_pago_seleccionado == 'venta') cobro @else pago @endif</label>
                    
                   <select {{$formato_ver == 0 ? 'disabled' : '' }}  wire:model='tipo_pago' wire:change='TipoPagoForm($event.target.value ,"{{$tipo_pago_seleccionado}}")'  class="form-control">
                    ...
                    @if($tipo_pago_seleccionado == 'venta' || $tipo_pago_seleccionado == 'venta_insumos')
                    <div class="form-group">
                   <label>Forma de cobro</label>
                   <select {{$formato_ver == 0 ? 'disabled' : '' }}  wire:model='metodo_pago_agregar_pago' wire:change='MetodoPagoForm($event.target.value ,"{{$tipo_pago_seleccionado}}")'  class="form-control">
                 ...
                 <div class="col-sm-12 col-md-12">
                  ...
                  <div class="form-group">
                   ...
                   <input {{$formato_ver == 0 ? 'readonly' : '' }} type="text" class="form-control" wire:model.defer="nro_comprobante">
                 </div>
              ...
              @if(0 < count($deducciones))
                ...
                <div class="col-sm-12 col-md-12">
                    ...
                    <a href="#" wire:click.prevent="toggleMostrarDeducciones">
                        {{ $mostrarDeducciones ? 'Ocultar Comisiones' : 'Ver Comisiones' }}
                    </a>
                                ...
                                <table class="table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th {{$formato_ver == 0 ? 'hidden' : '' }} >Acciones</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($deducciones as $index => $deduccion)
                                            <tr>
                                                <td>
                                                    <input {{$formato_ver == 0 ? 'disabled' : '' }} style="padding: 0rem !important;" type="text" wire:model.defer="deducciones.{{ $index }}.concepto" class="form-control" />
                                                </td>
                                                <td class="text-center">
                                                    <input {{$formato_ver == 0 ? 'disabled' : '' }}
                ...
                @endif
                <a hidden wire:click="AgregarDeduccion" href="javascript:void(0)">+ Agregar Deducción</a>
              </div>
              ...
              <div class="modal-footer">
                <br>
                <a href="javascript:void(0);" wire:click.prevent="CerrarVerPago()" class="btn btn-cancel" data-dismiss="modal">Cerrar</a>
                @if($formato_ver != 0)
                <a href="javascript:void(0);" wire:click.prevent="UpdatePago({{$id_pago}},'{{$tipo_pago_seleccionado}}')" class="btn btn-submit" data-dismiss="modal">Actualizar</a>
                @endif
